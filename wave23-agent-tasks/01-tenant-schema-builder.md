# Agent 1: Tenant Schema Builder

## Role
Create 8 database tables and migration 0035 for multi-tenant architecture including tenants, members, invitations, RBAC permissions, subscription plans, and API rate limits.

## Priority: WAVE 23 (Start Immediately)

## Files to CREATE

### 1. `docker/migrations/0035_multi_tenant.sql`
**Purpose**: 8 new tables for multi-tenant infrastructure
...
- [ ] Write complete migration SQL with all 8 tables, indexes, seed data for permissions and plans

### 2. `server/src/schema.ts` additions
- [ ] Add 8 new `sqliteTable` definitions matching migration
- [ ] Export all 8 tables

### 3. `server/src/db/postgres-schema.ts` additions
- [ ] Add 8 new `pgTable` definitions matching migration with PostgreSQL types
- [ ] Export all 8 tables

## Files to MODIFY

### 4. `server/src/schema.ts`
- [ ] Add exports for all 8 new tables at end of file
...
- [ ] 16 default permissions seeded
- [ ] 4 subscription plans seeded with correct limits
- [ ] FK cascades work (delete tenant cascades to members, invitations, role_permissions, subscriptions, rate_limits)
- [ ] Create marker file: `.agent-done-W23-01`

## Dependencies
- **None** -- can start immediately
- **Schema lock**: Only this agent may modify schema.ts and postgres-schema.ts in Wave 23
