#!/bin/bash
# ============================================================
# GoldLedger — Wave 10: Accounts Payable & Purchase Orders
# ============================================================
# Spawns Claude Code with Agent Teams in a tmux session.
# Run from WSL: bash launch-wave10.sh
# ============================================================
...
TMUX_SESSION="goldledger-wave10"
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"

echo -e "${CYAN}============================================================${NC}"
echo -e "${CYAN}  GoldLedger — Wave 10 Agent Team Launcher${NC}"
echo -e "${CYAN}  Accounts Payable & Purchase Orders${NC}"
echo -e "${CYAN}============================================================${NC}"
echo ""
...
# Step 3: Verify Wave 3 completion (Wave 10 depends on Wave 3, NOT Wave 9)
...
tmux set-option -t "$TMUX_SESSION" -g status-left "#[fg=#7aa2f7,bold] Wave 10: Accounts Payable & POs "
...
PROMPT="Read wave10-orchestration-prompt.md and execute the full agent team plan. Read each agent's task file from wave10-agent-tasks/ for detailed atomic tasks with file paths and specs. Spawn all 10 teammates according to the sub-wave execution order. Reference docs/wave0-master-plan.md for overall context. CRITICAL: Wave 10 depends on Wave 3 (NOT Wave 9) — it runs in parallel with Invoicing and Payroll tracks."
tmux send-keys -t "$TMUX_SESSION" "CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS=1 claude --dangerously-skip-permissions \"$PROMPT\"" C-m

echo ""
echo -e "${GREEN}============================================================${NC}"
echo -e "${GREEN}  ✅ Wave 10 Agent Team Launched!${NC}"
echo -e "${GREEN}============================================================${NC}"
echo ""
...
echo ""
echo -e "${YELLOW}  NOTE: Wave 10 can run IN PARALLEL with Waves 4-9${NC}"
echo -e "${YELLOW}  It only requires Wave 3 (Multi-User Cognee) to be complete.${NC}"
echo ""
