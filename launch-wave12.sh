#!/bin/bash
# ============================================================
# GoldLedger — Wave 12: Fixed Assets & Multi-Entity
# ============================================================
# Spawns Claude Code with Agent Teams in a tmux session.
# Run from WSL: bash launch-wave12.sh
# ============================================================
...
TMUX_SESSION="goldledger-wave12"
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"

echo -e "${CYAN}============================================================${NC}"
echo -e "${CYAN}  GoldLedger — Wave 12 Agent Team Launcher${NC}"
echo -e "${CYAN}  Fixed Assets & Multi-Entity${NC}"
echo -e "${CYAN}============================================================${NC}"
echo ""
...
# Step 3: Verify Wave 11 completion
echo -e "${YELLOW}[3/5] Checking Wave 11 prerequisites...${NC}"
if [ -f "$PROJECT_DIR/.agent-done-wave11" ]; then
    echo -e "${GREEN}  ✓ Wave 11 completed${NC}"
else
    echo -e "${YELLOW}  ⚠ Wave 11 marker not found — proceeding anyway${NC}"
fi
...
tmux set-option -t "$TMUX_SESSION" -g status-left "#[fg=#f7768e,bold] Wave 12: Fixed Assets & Multi-Entity "
