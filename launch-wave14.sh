#!/bin/bash
# ============================================================
# GoldLedger — Wave 14: AI Document Processing & Payment Matching
# ============================================================
# Spawns Claude Code with Agent Teams in a tmux session.
# Run from WSL: bash launch-wave14.sh
# ============================================================
...
TMUX_SESSION="goldledger-wave14"
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"

echo -e "${CYAN}============================================================${NC}"
echo -e "${CYAN}  GoldLedger — Wave 14 Agent Team Launcher${NC}"
echo -e "${CYAN}  AI Document Processing & Payment Matching${NC}"
echo -e "${CYAN}============================================================${NC}"
echo ""
...
# Step 3: Verify Wave 7 + Wave 10 completion
echo -e "${YELLOW}[3/5] Checking Wave 7 + Wave 10 prerequisites...${NC}"
if [ -f "$PROJECT_DIR/.agent-done-wave7" ]; then
    echo -e "${GREEN}  ✓ Wave 7 completed${NC}"
else
    echo -e "${YELLOW}  ⚠ Wave 7 marker not found — proceeding anyway${NC}"
fi
if [ -f "$PROJECT_DIR/.agent-done-wave10" ]; then
    echo -e "${GREEN}  ✓ Wave 10 completed${NC}"
else
    echo -e "${YELLOW}  ⚠ Wave 10 marker not found — proceeding anyway${NC}"
fi
...
tmux set-option -t "$TMUX_SESSION" -g status-left "#[fg=#f7768e,bold] Wave 14: AI DocProcess & Payment Match "
