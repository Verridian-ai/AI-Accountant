#!/bin/bash
# ============================================================
# GoldLedger — Wave 6: STP Compliance & Payroll Reporting
# ============================================================
# Spawns Claude Code with Agent Teams in a tmux session.
# Run from WSL: bash launch-wave6.sh
# ============================================================
...
TMUX_SESSION="goldledger-wave6"
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"

echo -e "${CYAN}============================================================${NC}"
echo -e "${CYAN}  GoldLedger — Wave 6 Agent Team Launcher${NC}"
echo -e "${CYAN}  STP Compliance & Payroll Reporting${NC}"
echo -e "${CYAN}============================================================${NC}"
echo ""
...
# Step 3: Verify Wave 5 completion
echo -e "${YELLOW}[3/5] Checking Wave 5 prerequisites...${NC}"
if [ -f "$PROJECT_DIR/.agent-done-W05-10" ]; then
    echo -e "${GREEN}  ✓ Wave 5 completed${NC}"
else
    echo -e "${YELLOW}  ⚠ Wave 5 marker not found — proceeding anyway${NC}"
fi
...
tmux set-option -t "$TMUX_SESSION" -g status-left "#[fg=#f7768e,bold] Wave 6: STP & Reporting "
