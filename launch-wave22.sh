#!/bin/bash
# ============================================================
# GoldLedger — Wave 22: Advanced Visualizations & Chart Library
# ============================================================
# Spawns Claude Code with Agent Teams in a tmux session.
# Run from WSL: bash launch-wave22.sh
# ============================================================
...
TMUX_SESSION="goldledger-wave22"
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"

echo -e "${CYAN}============================================================${NC}"
echo -e "${CYAN}  GoldLedger — Wave 22 Agent Team Launcher${NC}"
echo -e "${CYAN}  Advanced Visualizations & Chart Library${NC}"
echo -e "${CYAN}============================================================${NC}"
echo ""
...
# Step 3: Verify Wave 13 + Wave 15 completion
echo -e "${YELLOW}[3/5] Checking Wave 13 + Wave 15 prerequisites...${NC}"
if [ -f "$PROJECT_DIR/.agent-done-wave13" ]; then
    echo -e "${GREEN}  ✓ Wave 13 completed${NC}"
else
    echo -e "${YELLOW}  ⚠ Wave 13 marker not found — proceeding anyway${NC}"
fi
if [ -f "$PROJECT_DIR/.agent-done-wave15" ]; then
    echo -e "${GREEN}  ✓ Wave 15 completed${NC}"
else
    echo -e "${YELLOW}  ⚠ Wave 15 marker not found — proceeding anyway${NC}"
fi
...
tmux set-option -t "$TMUX_SESSION" -g status-left "#[fg=#f7768e,bold] Wave 22: Advanced Viz & Chart Library "
