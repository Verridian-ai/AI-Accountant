#!/bin/bash
# ============================================================
# GoldLedger — Wave 5: Pay Run Processing & Leave Management
# ============================================================
# Spawns Claude Code with Agent Teams in a tmux session.
# Run from WSL: bash launch-wave5.sh
# ============================================================
...
TMUX_SESSION="goldledger-wave5"
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"

echo -e "${CYAN}============================================================${NC}"
echo -e "${CYAN}  GoldLedger — Wave 5 Agent Team Launcher${NC}"
echo -e "${CYAN}  Pay Run Processing & Leave Management${NC}"
echo -e "${CYAN}============================================================${NC}"
echo ""
...
# Step 3: Verify Wave 4 completion
echo -e "${YELLOW}[3/5] Checking Wave 4 prerequisites...${NC}"
if [ -f "$PROJECT_DIR/.agent-done-wave4" ]; then
    echo -e "${GREEN}  ✓ Wave 4 completed${NC}"
else
    echo -e "${YELLOW}  ⚠ Wave 4 marker not found — proceeding anyway${NC}"
fi
...
tmux set-option -t "$TMUX_SESSION" -g status-left "#[fg=#f7768e,bold] Wave 5: Pay Runs & Leave "
