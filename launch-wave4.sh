#!/bin/bash
# ============================================================
# GoldLedger — Wave 4: Employee Management & Pay Structures
# ============================================================
# Spawns Claude Code with Agent Teams in a tmux session.
# Run from WSL: bash launch-wave4.sh
# ============================================================
...
TMUX_SESSION="goldledger-wave4"
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"

echo -e "${CYAN}============================================================${NC}"
echo -e "${CYAN}  GoldLedger — Wave 4 Agent Team Launcher${NC}"
echo -e "${CYAN}  Employee Management & Pay Structures${NC}"
echo -e "${CYAN}============================================================${NC}"
echo ""
...
# Step 3: Verify Wave 3 completion
echo -e "${YELLOW}[3/5] Checking Wave 3 prerequisites...${NC}"
if [ -f "$PROJECT_DIR/.agent-done-wave3" ]; then
    echo -e "${GREEN}  ✓ Wave 3 completed${NC}"
else
    echo -e "${YELLOW}  ⚠ Wave 3 marker not found — proceeding anyway${NC}"
fi

# Check TFN encryption key
if [ -z "$TFN_ENCRYPTION_KEY" ]; then
    echo -e "${YELLOW}  ⚠ TFN_ENCRYPTION_KEY not set — TFN encryption will be disabled${NC}"
    echo -e "${YELLOW}    Generate with: export TFN_ENCRYPTION_KEY=\$(openssl rand -hex 32)${NC}"
else
    echo -e "${GREEN}  ✓ TFN_ENCRYPTION_KEY configured${NC}"
fi
...
tmux set-option -t "$TMUX_SESSION" -g status-left "#[fg=#f7768e,bold] Wave 4: Employee Management "
...
PROMPT="Read wave4-orchestration-prompt.md and execute the full agent team plan. Read each agent's task file from wave4-agent-tasks/ for detailed atomic tasks with file paths and specs. Spawn all 10 teammates according to the sub-wave execution order. Reference docs/wave0-master-plan.md for overall context. IMPORTANT: TFN encryption with AES-256-GCM is a SECURITY REQUIREMENT."
tmux send-keys -t "$TMUX_SESSION" "CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS=1 claude --dangerously-skip-permissions \"$PROMPT\"" C-m

echo ""
echo -e "${GREEN}============================================================${NC}"
echo -e "${GREEN}  ✅ Wave 4 Agent Team Launched!${NC}"
echo -e "${GREEN}============================================================${NC}"
echo ""
