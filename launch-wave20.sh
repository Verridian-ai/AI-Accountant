#!/bin/bash
# ============================================================
# GoldLedger — Wave 20: Admin Backend & System Dashboard
# ============================================================
# Spawns Claude Code with Agent Teams in a tmux session.
# Run from WSL: bash launch-wave20.sh
# ============================================================
...
TMUX_SESSION="goldledger-wave20"
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"

echo -e "${CYAN}============================================================${NC}"
echo -e "${CYAN}  GoldLedger — Wave 20 Agent Team Launcher${NC}"
echo -e "${CYAN}  Admin Backend & System Dashboard${NC}"
echo -e "${CYAN}============================================================${NC}"
echo ""
...
# Step 3: Verify Wave 16 + Wave 17 completion
echo -e "${YELLOW}[3/5] Checking Wave 16 + Wave 17 prerequisites...${NC}"
if [ -f "$PROJECT_DIR/.agent-done-wave16" ]; then
    echo -e "${GREEN}  ✓ Wave 16 completed${NC}"
else
    echo -e "${YELLOW}  ⚠ Wave 16 marker not found — proceeding anyway${NC}"
fi
if [ -f "$PROJECT_DIR/.agent-done-wave17" ]; then
    echo -e "${GREEN}  ✓ Wave 17 completed${NC}"
else
    echo -e "${YELLOW}  ⚠ Wave 17 marker not found — proceeding anyway${NC}"
fi
...
tmux set-option -t "$TMUX_SESSION" -g status-left "#[fg=#f7768e,bold] Wave 20: Admin Backend & System Dash "
