...
import { taxBrackets, cgtAssets, cgtEvents, depreciableAssets } from '../schema.js';
import crypto from 'crypto';

const gstTaxRoutes = new Hono();
...
gstTaxRoutes.post('/transactions/categorize-gst', async (c) => {
  try {
...
// POST /api/gst/bulk-approve
gstTaxRoutes.post('/gst/bulk-approve', async (c) => {
  try {
    const payload = c.get('jwtPayload');
    const userId = payload.userId;
...
// GET /api/gst/input-tax-credits
gstTaxRoutes.get('/gst/input-tax-credits', async (c) => {
  try {
    const payload = c.get('jwtPayload');
    const userId = payload.userId;
...
gstTaxRoutes.get('/tax/brackets/:year', async (c) => {
  try {
...
// GET /api/tax/assets
gstTaxRoutes.get('/tax/assets', async (c) => {
  try {
    const payload = c.get('jwtPayload');
    const userId = payload.userId;
...
// POST /api/tax/assets
gstTaxRoutes.post('/tax/assets', async (c) => {
  try {
    const payload = c.get('jwtPayload');
    const userId = payload.userId;
    const body = await c.req.json();
...
// GET /api/tax/cgt
gstTaxRoutes.get('/tax/cgt', async (c) => {
  try {
    const payload = c.get('jwtPayload');
    const userId = payload.userId;
...
// POST /api/tax/cgt/disposal
gstTaxRoutes.post('/tax/cgt/disposal', async (c) => {
  try {
    const payload = c.get('jwtPayload');
    const userId = payload.userId;
    const body = await c.req.json();
...
// GET /api/tax/depreciation/assets
gstTaxRoutes.get('/tax/depreciation/assets', async (c) => {
  try {
    const payload = c.get('jwtPayload');
    const userId = payload.userId;
...
// POST /api/tax/depreciation/assets
gstTaxRoutes.post('/tax/depreciation/assets', async (c) => {
  try {
    const payload = c.get('jwtPayload');
    const userId = payload.userId;
    const body = await c.req.json();
...
// GET /api/tax/depreciation/calculate/:assetId
gstTaxRoutes.get('/tax/depreciation/calculate/:assetId', async (c) => {
  try {
    const payload = c.get('jwtPayload');
    const userId = payload.userId;
...
void basService; // imported for side-effects (service init)

export default gstTaxRoutes;
