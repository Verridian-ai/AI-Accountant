...
const _indicatorQueryShape = z.object({ category: z.string().optional(), source: z.string().optional() });
...
indicatorRoutes.get('/snapshot', async (c) => {
  ...
  for (const ind of indicators) {
    ...
    if (!existing || ind.observationDate > existing.observationDate) latestMap.set(ind.indicatorCode, ind);
  }
...
indicatorRoutes.get('/cpi', async (c) => {
  return c.json({ abs: { allGroups: await absDataFeed.getLatestIndicator('ABS_CPI_ALL_GROUPS'), allGroupsPct: await absDataFeed.getLatestIndicator('ABS_CPI_ALL_GROUPS_PCT') }, rba: { quarterly: await rbaDataFeed.getRateHistory('RBA_CPI_QUARTERLY', 12) } });
});
...
indicatorRoutes.get('/', async (c) => {
  ...
  const rows = await db.select().from(economicIndicators).where(conditions.length > 0 ? and(...conditions) : undefined).orderBy(desc(economicIndicators.observationDate)).limit(100).all();
  return c.json({ indicators: rows, count: rows.length });
});

indicatorRoutes.get('/:code/history', async (c) => {
  const code = c.req.param('code');
  const history = [...(await rbaDataFeed.getRateHistory(code, 24)).map(h => ({ ...h, source: 'rba' })), ...(await absDataFeed.getIndicatorHistory(code, 24)).map(h => ({ date: h.observationDate, value: h.value, source: 'abs' }))].sort((a, b) => b.date.localeCompare(a.date));
