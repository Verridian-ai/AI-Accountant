...
export function registerBASHandlers(app: Hono): void {
  // Get available BAS quarters
  app.get('/quarters', async (c) => {
    try {
  ...
  // Get BAS calculation
  app.get('/:quarter/calculate', async (c) => {
    try {
      const payload = c.get('jwtPayload');
  ...
  app.post('/:quarter/save', zValidator('json', saveBASSchema), async (c) => {
    try {
      const payload = c.get('jwtPayload');
      const quarter = c.req.param('quarter');
  ...
  // BAS Status Update
  app.patch('/:quarter/status', zValidator('json', updateBASStatusSchema), async (c) => {
    try {
      const payload = c.get('jwtPayload');
      const userId = payload.userId;
      const quarter = c.req.param('quarter');
  ...
  // BAS Drill-Down
  app.get('/:quarter/drill-down/:label', async (c) => {
    try {
      const payload = c.get('jwtPayload');
      const quarter = c.req.param('quarter');
