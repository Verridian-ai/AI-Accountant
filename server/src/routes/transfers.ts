...
transferRoutes.post('/detect', async (c) => {
  ...
  const existingPairs = existingLinks.map((l: Record<string, unknown>) => ({ sourceId: l.sourceTransactionId, targetId: l.destinationTransactionId })); // eslint-disable-line @typescript-eslint/no-explicit-any
  const detector = new TransferDetector();
  const matches = detector.detectTransfers(allTxs as unknown as TransferCandidate[], userAccounts as unknown as AccountContext[], existingPairs);
  ...
  for (const m of matches) {
    const src = userAccounts.find((a: Record<string, unknown>) => a.id === m.sourceTransaction.accountId); // eslint-disable-line @typescript-eslint/no-explicit-any
    const dst = userAccounts.find((a: Record<string, unknown>) => a.id === m.targetTransaction.accountId); // eslint-disable-line @typescript-eslint/no-explicit-any
