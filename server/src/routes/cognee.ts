...
import { getUserId, getTenantId } from '../utils/auth-helpers.js';
...
cogneeRoutes.post('/init-user', zValidator('json', initCogneeUserSchema), async (c) => {
  ...
  await db
    .insert(cogneeUserAccounts)
    .values({
      id: crypto.randomUUID(),
      userId,
      cogneeEmail,
      cogneeRefreshToken: result.refreshToken,
    ...
    .run();
  if (result.userId && result.refreshToken)
    await cogneeClient.setupUserAuth(userId, result.refreshToken);
...
cogneeRoutes.get('/session', async (c) => {
  ...
  return c.json(
    await cogneeSessionService.getOrCreateCogneeSession(userId, {
      sessionType: 'chat',
      datasetPrefix,
    }),
  );
