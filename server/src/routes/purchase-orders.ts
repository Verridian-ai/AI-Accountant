...
const receiveGoodsSchema = z.object({
  ...
  notes: z.string().max(2000).optional(),
  lines: z
    .array(z.object({ poLineId: z.string().min(1), quantityReceived: z.number().min(0) }))
    .min(1),
});
...
poRoutes.get('/', zValidator('query', paginationSchema), async (c) => {
...
poRoutes.post('/', zValidator('json', createPOSchema), async (c) => {
  try {
    return c.json(
      { data: await purchaseOrderService.createPurchaseOrder(getUserId(c), c.req.valid('json')) },
      201,
    );
  } catch {
...
poRoutes.patch('/:id', zValidator('json', updatePOSchema), async (c) => {
  try {
    return c.json({
      data: await purchaseOrderService.updatePurchaseOrder(c.req.param('id'), c.req.valid('json')),
    });
  } catch {
    return c.json({ error: 'Failed' }, 400);
  }
...
poRoutes.post('/:id/receive', zValidator('json', receiveGoodsSchema), async (c) => {
  try {
    return c.json({
      data: await purchaseOrderService.receiveGoods(c.req.param('id'), c.req.valid('json')),
    });
  } catch {
    return c.json({ error: 'Failed' }, 400);
  }
...
poRoutes.post('/payment-runs', zValidator('json', createPaymentRunSchema), async (c) => {
  try {
    return c.json(
      { data: await purchaseOrderService.createPaymentRun(getUserId(c), c.req.valid('json')) },
      201,
    );
  } catch {
    return c.json({ error: 'Failed' }, 400);
