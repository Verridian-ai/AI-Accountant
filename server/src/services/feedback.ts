...
export interface ParserImprovementSuggestion {
    type: 'category_rule' | 'merchant_mapping' | 'date_format' | 'amount_parsing' | 'duplicate_detection';
...
export class FeedbackManager {
    ...
    async submitFeedback(
        ...
        logger.info(`[FeedbackManager] Submitting ${feedbackType} feedback for transaction ${transactionId}`);
    ...
    async updateFeedbackStatus(
        ...
        await db.update(parserFeedback)
            .set({
                status,
    ...
    private calculatePriority(pattern: FeedbackPattern): 'high' | 'medium' | 'low' {
        ...
        if (pattern.occurrences >= 10 || (pattern.avgConfidence !== null && pattern.avgConfidence < 0.5)) {
            return 'high';
        }
    ...
    async getFeedbackStats(userId?: string): Promise<FeedbackStats> {
        ...
        for (const status of Object.keys(statusCounts) as FeedbackStatus[]) {
            const result = await db.select({ count: count() })
                ...
                .where(and(
                    ...conditions,
                    eq(parserFeedback.status, status)
                ))
                .get();
        ...
        for (const feedbackType of Object.keys(typeCounts) as FeedbackType[]) {
            const result = await db.select({ count: count() })
                ...
                .where(and(
                    ...conditions,
                    eq(parserFeedback.feedbackType, feedbackType)
                ))
                .get();
        ...
        const appliedTodayResult = await db.select({ count: count() })
            .from(parserFeedback)
            .where(and(
                ...conditions,
