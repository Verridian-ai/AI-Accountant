...
export class BatchSizeExceededError extends QueueError {
    constructor(size: number, maxSize: number) {
        super(`Batch size ${size} exceeds maximum of ${maxSize}`, 'BATCH_SIZE_EXCEEDED', { size, maxSize });
    }
}
...
export class BulkUploadQueue {
    ...
    async addJob(files: FileInfo[], userId: string, options: AddJobOptions = {}): Promise<QueueJob> {
        ...
        for (const file of files) {
            ...
            if (!filePath.startsWith(baseDir)) {
                throw new QueueError(`Invalid filename: ${file.name}`, 'INVALID_FILENAME', { filename: file.name });
            }
    ...
    getStats(): QueueStats {
        ...
        for (const job of completedJobs) {
            if (job.startedAt && job.completedAt) {
                totalProcessingTime += new Date(job.completedAt).getTime() - new Date(job.startedAt).getTime();
                processedCount++;
            }
        }
    ...
    private async processFileWithWorker(worker: Worker, job: QueueJob, file: JobFile): Promise<void> {
        logger.info(`[Queue] Worker ${worker.id} processing file ${file.originalName} from job ${job.id}`);
        ...
        const timeoutPromise = new Promise<never>((_, reject) => {
            setTimeout(() => {
                reject(new QueueError(`File processing timed out after ${this.config.jobTimeoutMs}ms`, 'PROCESSING_TIMEOUT'));
        ...
        try {
            ...
            if (canRetry) {
                ...
                const delay = this.config.retryDelayMs * Math.pow(this.config.retryBackoffMultiplier, file.retryCount - 1);
                logger.info(`[Queue] File ${file.originalName} will retry in ${delay}ms (attempt ${file.retryCount + 1})`);
    ...
    private async checkJobCompletion(job: QueueJob): Promise<void> {
        ...
        if (pendingFiles.length === 0) {
            ...
            logger.info(`[Queue] Job ${job.id} ${job.state}: ${job.progress.completed} completed, ${job.progress.failed} failed`);
        }
    }
