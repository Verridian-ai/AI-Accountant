...
import type {
  ProfitAndLossReport, PeriodComparison, KPIMetrics, ReportData,
} from './types.js';
...
export class ReportAnalytics {
  ...
  async comparePeriods(
    ...
    switch (reportType) {
      case 'profit_and_loss':
        ...
        priorData = await this.reports.generateProfitAndLoss(userId, priorStart, priorEnd); break;
      case 'cash_flow':
        currentData = await this.reports.generateCashFlow(userId, currentStart, currentEnd);
        priorData = await this.reports.generateCashFlow(userId, priorStart, priorEnd); break;
      case 'balance_sheet':
        currentData = await this.reports.generateBalanceSheet(userId, currentEnd);
        priorData = await this.reports.generateBalanceSheet(userId, priorEnd); break;
      case 'trial_balance':
        currentData = await this.reports.generateTrialBalance(userId, currentEnd);
        priorData = await this.reports.generateTrialBalance(userId, priorEnd); break;
      default:
        currentData = await this.reports.generateProfitAndLoss(userId, currentStart, currentEnd);
  ...
  async createSnapshot(
    ...
    await db.insert(reportSnapshots).values({
      id, templateId, userId: userId ?? 'system', reportType: reportType ?? 'custom',
      periodStart: periodStart ?? now.split('T')[0], periodEnd: periodEnd ?? now.split('T')[0],
      data: JSON.stringify(reportData),
      metadata: JSON.stringify({ generatedAt: now, rowCount: Array.isArray(reportData) ? reportData.length : 1 }),
      generatedAt: now,
    }).run();
    return id;
  }
  ...
  async getKPIs(userId: string, period: string): Promise<KPIMetrics> {
    ...
    try { priorPnl = await this.reports.generateProfitAndLoss(userId, prior.start, prior.end); } catch { /* no prior data */ }
    ...
    const revenueGrowth = priorPnl ? safeDivide(pnl.grossRevenue - priorPnl.grossRevenue, priorPnl.grossRevenue) * 100 : 0;
    ...
    const previousMetrics = await db.select({ metricName: kpiMetrics.metricName, metricValue: kpiMetrics.metricValue })
      .from(kpiMetrics)
      .where(and(eq(kpiMetrics.userId, userId), sql`${kpiMetrics.period} < ${period}`))
      .orderBy(sql`${kpiMetrics.period} DESC`).all();

    const previousMap = new Map<string, number>();
    for (const pm of previousMetrics) { if (!previousMap.has(pm.metricName)) previousMap.set(pm.metricName, Number(pm.metricValue)); }

    const resultMetrics: KPIMetrics['metrics'] = [];
    for (const def of metricDefs) {
      const prevValue = previousMap.get(def.name) ?? null;
      let trend: 'up' | 'down' | 'stable' = 'stable';
      if (prevValue !== null) { if (def.value > prevValue) trend = 'up'; else if (def.value < prevValue) trend = 'down'; }
      resultMetrics.push({ metricName: def.name, value: Math.round(def.value * 100) / 100, target: def.target, trend, previousValue: prevValue });

      const existingMetric = await db.select().from(kpiMetrics)
        .where(and(eq(kpiMetrics.userId, userId), eq(kpiMetrics.metricName, def.name), eq(kpiMetrics.period, period))).get();

      if (existingMetric) {
        await db.update(kpiMetrics).set({
          metricValue: Math.round(def.value * 100) / 100, targetValue: def.target,
          trendDirection: trend, previousValue: prevValue, calculatedAt: new Date().toISOString(),
        }).where(eq(kpiMetrics.id, existingMetric.id)).run();
      } else {
        await db.insert(kpiMetrics).values({
          id: crypto.randomUUID(), userId, metricName: def.name,
          metricValue: Math.round(def.value * 100) / 100, period, targetValue: def.target,
          trendDirection: trend, previousValue: prevValue, calculatedAt: new Date().toISOString(),
        }).run();
      }
    }
...
export function parsePeriodRange(period: string): { periodStart: string; periodEnd: string } {
  ...
  if (period.length === 7) {
    ...
    return { periodStart: `${period}-01`, periodEnd: `${period}-${String(lastDay).padStart(2, '0')}` };
  }
