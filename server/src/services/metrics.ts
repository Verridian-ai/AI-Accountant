import { db, parserMetrics, parserAccuracyAggregates, statements, parserFeedback } from '../schema.js';
...
export class ParserMetrics {
    ...
    async getParserAccuracy(parserId: string, dateRange?: DateRange): Promise<ParserAccuracyResult> {
        ...
        const result = await db.select({
            totalAttempts: count(),
            successfulAttempts: sum(sql<number>`CASE WHEN ${parserMetrics.parseErrorCount} = 0 THEN 1 ELSE 0 END`),
            failedAttempts: sum(sql<number>`CASE WHEN ${parserMetrics.parseErrorCount} > 0 THEN 1 ELSE 0 END`),
    ...
    async getAggregatedMetrics(dateRange?: DateRange): Promise<AggregatedMetrics[]> {
        ...
        const rawMetrics = await db.select({
        ...
        for (const metric of rawMetrics) {
            ...
            const durations = await db.select({
            ...
            const totalConfidenceItems = Number(metric.totalHighConfidence || 0) + Number(metric.totalLowConfidence || 0);
            ...
            results.push({
                ...
                highConfidenceRate: totalConfidenceItems > 0
                    ? Number(metric.totalHighConfidence || 0) / totalConfidenceItems
                    : null,
                lowConfidenceRate: totalConfidenceItems > 0
    ...
    async computeRollups(): Promise<{ daily: number; weekly: number; monthly: number }> {
        ...
        const banks = await db.selectDistinct({ bankId: parserMetrics.bankId })
            .from(parserMetrics)
            .all();
    ...
    async getTopErrors(limit: number = 10, dateRange?: DateRange): Promise<TopError[]> {
        ...
        for (const error of errors) {
            ...
            const parsers = await db.selectDistinct({
                bankId: parserMetrics.bankId
            })
                .from(parserMetrics)
    ...
    private async computeAndSaveRollup(
        ...
        if (existingAgg) {
            await db.update(parserAccuracyAggregates)
                .set({
                    totalParseAttempts,
                    successfulParses,
                    failedParses,
                    totalTransactionsParsed,
                    avgTransactionsPerStatement: totalParseAttempts > 0 ? totalTransactionsParsed / totalParseAttempts : null,
                    avgDurationMs,
                    minDurationMs,
                    maxDurationMs,
                    p50DurationMs,
                    p95DurationMs,
                    avgConfidenceScore,
                    highConfidenceRate: totalConfidenceItems > 0 ? totalHighConfidence / totalConfidenceItems : null,
