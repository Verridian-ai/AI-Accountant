...
export function ruleCategorize(
  ...
  // --- Interest patterns ---
  if (desc.includes('interest') && amount > 0)
    return { suggestedCategory: 'Interest Income', confidence: 0.85 };
  if (desc.includes('interest') && amount < 0)
    return { suggestedCategory: 'Interest Expense', confidence: 0.85 };
  ...
  if (desc.includes('officeworks'))
    return { suggestedCategory: 'Office Supplies', confidence: 0.9 };
  ...
  if (desc.includes('afterpay'))
    return { suggestedCategory: 'Cost of Goods Sold', confidence: 0.85 };
