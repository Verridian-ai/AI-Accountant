/**
 * Vercel AI SDK -- Budget Analyzer Agent
 *
 * Drop-in replacement for BudgetAnalyzerAgent (legacy ClaudeAgent)
...
export class VercelBudgetAnalyzer extends VercelAgent<BudgetAnalyzerInput, BudgetAnalyzerOutput> {
  constructor() {
  ...
  getTools(sessionId?: string): ToolSet {
    ...
    tools['search_financial_context'] = adaptLegacyTool(
      ...
      async (input) => {
        ...
        try {
          return await cogneeTools.search(
            query,
            'financial_insights',
            'GRAPH_COMPLETION',
            sessionId,
  ...
  buildPrompt(input: BudgetAnalyzerInput): string {
    ...
    if (input.includeProjections) {
      parts.push('Include future balance projections.');
    }

    parts.push(
      '\nReturn a complete BudgetAnalyzerOutput with insights, categoryBreakdown, recurringExpenses, savingsRate, and summary.',
    );
   ...
   * returning empty arrays and a summary describing the failure.
   */
  protected override buildFallbackOutput(
    _input: BudgetAnalyzerInput,
    _error: unknown,
