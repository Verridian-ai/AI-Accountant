...
export class PaymentMatchingAgent extends ClaudeAgent<
  PaymentMatchingInput,
  PaymentMatchingOutput
> {
  ...
  protected tools: Anthropic.Tool[] = [
    {
      ...
      description: 'Find potential bank transaction matches for an OCR-extracted document. Returns candidates sorted by confidence score descending.',
      input_schema: {
        type: 'object' as const,
        properties: {
          documentId: { type: 'string', description: 'The UUID of the OCR document to find matches for' },
          amountTolerance: { type: 'number', description: 'Amount tolerance in dollars for fuzzy matching (default: 1.0)' },
          dateTolerance: { type: 'number', description: 'Date tolerance in days for temporal matching (default: 30)' },
    ...
    {
      name: 'score_match',
      description: 'Calculate a detailed match score between an OCR document and a specific bank transaction. Returns per-factor breakdown and overall confidence.',
      input_schema: {
        type: 'object' as const,
        properties: {
          documentId: { type: 'string', description: 'The UUID of the OCR document' },
          transactionId: { type: 'string', description: 'The ID of the bank transaction to score against' },
        },
    ...
    },
    {
      ...
      description: 'Apply user-defined matching rules to find definitive matches for a document. Returns the best rule-based match or null if no rules match.',
      input_schema: {
        type: 'object' as const,
        properties: {
          documentId: { type: 'string', description: 'The UUID of the OCR document to apply rules against' },
        },
        required: ['documentId'],
      },
    },
    {
      ...
      description: 'Learn from a confirmed match to create or refine matching rules. Analyzes the match pattern and optionally creates a new rule for future matches.',
      input_schema: {
        type: 'object' as const,
        properties: {
          matchId: { type: 'string', description: 'The UUID of the confirmed payment match to learn from' },
        },
        required: ['matchId'],
      },
    },
  ...
  >([
    [
      ...
      async (input) => {
        ...
        const options: any = {};
        if (input.amountTolerance != null) options.amountTolerance = input.amountTolerance as number;
    ...
    [
      ...
      async (input) => {
        ...
        const doc = await db.select().from(ocrDocuments)
          .where(eq(ocrDocuments.id, documentId)).get();
        if (!doc) throw new Error(`Document not found: ${documentId}`);

        const tx = await db.select().from(transactions)
          .where(eq(transactions.id, transactionId)).get();
