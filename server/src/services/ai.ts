...
export class AIService {
    ...
    async parseWithVisionBatched(
    ...
    ): Promise<{ transactions: Array<{ date: string; description: string; amount_cents: number; balance_cents?: number }> }> {
        ...
        // Process in batches
        const allTransactions: Array<{ date: string; description: string; amount_cents: number; balance_cents?: number }> = [];
        ...
        for (let batchStart = 0; batchStart < pageBuffers.length; batchStart += pagesPerBatch) {
            ...
            console.log(`[AI Vision Batched] Processing batch ${batchNum}/${totalBatches} (pages ${batchStart + 1}-${batchEnd})`);
...
- Return ONLY valid JSON`;
            ...
            try {
                ...
                console.log(`[AI Vision Batched] Batch ${batchNum} extracted ${batchTxs.length} transactions`);
        ...
        console.log(`[AI Vision Batched] Total: ${allTransactions.length} raw, ${deduplicated.length} after dedup`);
