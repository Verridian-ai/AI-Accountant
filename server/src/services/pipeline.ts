...
} from './parsers/documents/credit-card/index.js';
...
async function flagLikelyTransfersByDescription(
      ...
      try {
        ...
        if (!accountInfo) {
          accountInfo = await withRetry(
            () => aiService.extractAccountInfo(pdfText, settings?.modelParsingText),
            2,
            1000,
      ...
      if (!rawData || rawData.transactions.length === 0) {
            ...
            if (rawData.transactions.length > 0) {
              ...
              if (toInsert.length > 0) {
                await db.transaction(async (tx: any) => {
                  ...
                  if (userId) {
                    const highConf = categorizations.filter(
              ...
              if (accountDetection.accountId) {
                await accountService.linkStatementToAccount(
                  statementId,
                  accountDetection.accountId,
                );
              }
              ...
              await db
                ...
                .set({
                  ...
                  closingBalance:
                    rawData.transactions[rawData.transactions.length - 1]?.balance_cents ?? null,
        ...
        try {
          rawData = await withRetry(
            async () => {
              const result = await aiService.parseStatementText(
                pdfText,
        ...
        if (userId && toInsert.length > 0) {
          try {
            ...
            if (userAccounts.length > 1 && allUserTxs.length > 1) {
              ...
              if (matches.length > 0) {
                logger.info(
                  `[Pipeline] Found ${matches.length} potential transfers, persisting...`,
                );
                ...
                for (const match of matches) {
                  const srcAcct = accountContexts.find(
                    (a) => a.id === match.sourceTransaction.accountId,
                  );
                  const dstAcct = accountContexts.find(
                    (a) => a.id === match.targetTransaction.accountId,
                  );
                  if (
                    srcAcct?.ownershipTag === 'personal' &&
                    dstAcct?.ownershipTag === 'business'
                  ) {
                ...
                if (ownerContribIds.length > 0) {
                  ...
                  logger.info(
                    `[Pipeline] Marked ${ownerUpdated} transactions as owner contributions`,
                  );
                }
