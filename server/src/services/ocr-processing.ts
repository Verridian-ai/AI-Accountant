...
const VALID_DOCUMENT_TYPES = new Set([
  'invoice', 'receipt', 'bill', 'credit_note', 'statement', 'quote', 'purchase_order', 'unknown',
]);

/** Keyword → category mapping for line item classification */
const CATEGORY_KEYWORDS: Record<string, string[]> = {
  'Office Supplies': ['stationery', 'paper', 'pen', 'ink', 'toner', 'cartridge', 'folder', 'office'],
  'Computer & IT': ['software', 'license', 'hosting', 'domain', 'computer', 'laptop', 'monitor', 'keyboard', 'mouse', 'ssd', 'ram', 'server'],
  'Utilities': ['electricity', 'gas', 'water', 'power', 'energy', 'utility'],
  'Telecommunications': ['phone', 'mobile', 'internet', 'broadband', 'nbn', 'telstra', 'optus', 'vodafone', 'sim'],
  'Rent & Occupancy': ['rent', 'lease', 'occupancy', 'tenancy'],
  'Insurance': ['insurance', 'premium', 'indemnity', 'liability cover'],
  'Motor Vehicle': ['fuel', 'petrol', 'diesel', 'rego', 'registration', 'car', 'vehicle', 'parking', 'toll'],
  'Travel & Accommodation': ['flight', 'airfare', 'hotel', 'motel', 'accommodation', 'taxi', 'uber', 'travel'],
  'Meals & Entertainment': ['meal', 'lunch', 'dinner', 'breakfast', 'coffee', 'catering', 'restaurant'],
  'Professional Services': ['accounting', 'legal', 'consulting', 'advisory', 'audit', 'bookkeeping'],
  'Advertising & Marketing': ['advertising', 'marketing', 'promotion', 'ad spend', 'google ads', 'facebook ads', 'seo'],
  'Repairs & Maintenance': ['repair', 'maintenance', 'service', 'fix', 'plumber', 'electrician'],
  'Cleaning': ['cleaning', 'cleaner', 'janitorial', 'sanitation'],
  'Postage & Freight': ['postage', 'freight', 'courier', 'shipping', 'delivery', 'auspost', 'sendle'],
...
export class OCRProcessingService {
  ...
  async uploadDocument(
    ...
    await db.insert(ocrDocuments).values({
      id: docId,
      userId,
...
Return ONLY valid JSON, no markdown.`,
      ...
      await db.update(ocrDocuments).set({
        ...
        processedAt: now,
        updatedAt: now,
      }).where(eq(ocrDocuments.id, documentId)).run();

      // Insert line items
      if (extraction.lineItems?.length) {
        for (const item of extraction.lineItems) {
          await db.insert(ocrLineItems).values({
            id: randomUUID(),
            documentId,
      ...
      logger.info(`[OCR] Document ${documentId} processed: ${extraction.lineItems?.length ?? 0} line items, confidence ${extraction.confidence}`);

      const updated = await db.select().from(ocrDocuments).where(eq(ocrDocuments.id, documentId)).get();
  ...
  async extractLineItems(documentId: string): Promise<OCRLineItemRecord[]> {
    ...
    let items: any[] = await db.select().from(ocrLineItems)
    ...
    if (!items.length) {
      ...
      items = await db.select().from(ocrLineItems)
        .where(eq(ocrLineItems.documentId, documentId))
        .orderBy(asc(ocrLineItems.lineNumber))
        .all();
    }
    ...
    for (const item of items) {
      ...
      if (userId) {
        const memories: any[] = await db.select().from(merchantMemory)
          .where(eq(merchantMemory.userId, userId))
          .all();

        for (const mem of memories) {
          if (item.description.toLowerCase().includes(mem.merchantPattern.toLowerCase())) {
            await db.update(ocrLineItems).set({ category: mem.category }).where(eq(ocrLineItems.id, item.id)).run();
      ...
      if (!matched) {
        ...
        for (const [category, keywords] of Object.entries(CATEGORY_KEYWORDS)) {
          if (keywords.some(kw => desc.includes(kw))) {
            await db.update(ocrLineItems).set({ category }).where(eq(ocrLineItems.id, item.id)).run();
  ...
  async enqueueDocument(documentId: string, action: string, priority: number = 100): Promise<void> {
    ...
    await db.insert(documentQueue).values({
      id: randomUUID(),
      userId: doc.userId,
      documentId,
      action,
  ...
  async processQueue(limit: number = 10): Promise<void> {
    ...
    const items: any[] = await db.select().from(documentQueue)
    ...
    for (const item of items) {
      ...
      // Mark as processing
      await db.update(documentQueue).set({
        status: 'processing',
        attempts: (item.attempts ?? 0) + 1,
        startedAt: now,
      }).where(eq(documentQueue.id, item.id)).run();
      ...
      try {
        ...
        if (attempts < maxAttempts) {
          ...
          logger.warn(`[OCR] Queue item ${item.id} failed (attempt ${attempts}/${maxAttempts}), will retry: ${err.message}`);
        } else {
          ...
          logger.error(`[OCR] Queue item ${item.id} permanently failed after ${attempts} attempts: ${err.message}`);
        }
      }
    }
  }
  ...
  async listDocuments(userId: string, status?: string, documentType?: string): Promise<OCRDocumentRecord[]> {
    let query = db.select().from(ocrDocuments).where(eq(ocrDocuments.userId, userId));

    if (status) {
      query = db.select().from(ocrDocuments).where(
        and(eq(ocrDocuments.userId, userId), eq(ocrDocuments.status, status))
      );
    }
