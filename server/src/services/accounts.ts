...
export class AccountService {
    ...
    async findAccountByHash(userId: string, accountNumberHash: string) {
        return db.select()
            ...
            .where(and(
                ...
                eq(accounts.accountNumberHash, accountNumberHash)
            ))
            .get();
    }
    ...
    async batchUpdateMerchantMemory(userId: string, updates: Array<{
        merchantPattern: string;
        merchantDisplayName?: string;
        category: string;
        gstApplicable: boolean;
        ...
        const existingMemories = await db.select()
            ...
            .where(and(
                ...
                inArray(merchantMemory.merchantPattern, patterns)
            ))
            .all();
