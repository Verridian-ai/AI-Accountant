...
// Dynamically import the CBA parser (ESM)
const { CBAParser } = await import('./dist/services/parsers/banks/cba.js').catch(async () => {
    // If dist doesn't exist, try tsx compile approach
    console.log('dist/ not found, trying direct import via tsx...');
    return import('./src/services/parsers/banks/cba.ts');
});
...
for (const filename of files) {
    ...
    try {
        ...
        let stmt = db.prepare('SELECT id, parsing_status, transaction_count FROM statements WHERE hash = ?').get(hash);
        ...
        // 4. Detect bank
        const confidence = parser.detectConfidence(pdfText);
        console.log(`  CBA confidence: ${(confidence * 100).toFixed(0)}%`);

        if (confidence < 0.3) {
            console.log(`  Not a CBA statement (confidence too low), marking as needs AI parsing`);
        ...
        const insertMany = db.transaction((txs) => {
            for (const tx of txs) {
                ...
                insertTx.run(
                    ...
                    null,                 // category - will be set by AI later
                    statementId,
                    userId,
                    txHash
                );
...
const allStmts = db.prepare('SELECT id, filename, parsing_status, transaction_count FROM statements ORDER BY filename').all();
console.log(`\nDB Summary (${allStmts.length} statements):`);
allStmts.forEach(s => {
    console.log(`  ${s.filename}: ${s.parsing_status} (${s.transaction_count || 0} txs)`);
});

const totalTx = db.prepare('SELECT COUNT(*) as cnt FROM transactions').get();
