...
export function TrialBalance({ asAtDate }: Props) {
    ...
    useEffect(() => {
        ...
        setLoading(true);
        setError(null);
        reportsApi.fetchTrialBalance(asAtDate)
            .then(setData)
    ...
    const SortHeader = ({ label, field }: { label: string; field: SortKey }) => (
        ...
        >
            <span className="flex items-center gap-1 justify-end">
                {label}
                <ArrowUpDown className={`h-3 w-3 ${sortKey === field ? 'text-[#FFCC00]' : 'text-zinc-600'}`} />
            </span>
        </th>
    );
                ...
                <table className="w-full text-sm">
                    <thead>
                        <tr className="text-zinc-500 border-b border-white/5">
                            <th className="text-left pb-2 font-medium cursor-pointer hover:text-[#FFCC00] transition-colors" onClick={() => handleSort('accountName')}>
                                <span className="flex items-center gap-1">
                                    Account
                                    <ArrowUpDown className={`h-3 w-3 ${sortKey === 'accountName' ? 'text-[#FFCC00]' : 'text-zinc-600'}`} />
                                </span>
                            </th>
                    ...
                    <tbody>
                        {entries.map((entry, idx) => (
                            <tr key={`${entry.accountName}-${idx}`} className="border-b border-white/5 hover:bg-white/5 transition-colors">
                                ...
                                <td className={`py-2 text-right font-mono ${entry.net >= 0 ? 'text-emerald-400' : 'text-red-400'}`}>
                    ...
                    <tfoot>
                        <tr className="border-t-2 border-[#FFCC00]/30">
                            ...
                            <td className="pt-3 text-right font-bold font-mono text-emerald-400">{formatCurrency(data.totalDebits)}</td>
                            <td className="pt-3 text-right font-bold font-mono text-red-400">{formatCurrency(data.totalCredits)}</td>
                            <td className={`pt-3 text-right font-bold font-mono ${data.difference === 0 ? 'text-[#FFCC00]' : 'text-red-400'}`}>
