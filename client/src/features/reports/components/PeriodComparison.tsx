          ...
          <div>
            ...
            <div className="flex gap-2">
              ...
              <button
                ...
                className="neu-raised px-4 py-2 rounded-xl text-sm font-bold text-cba-gold hover:bg-cba-gold/10 disabled:opacity-40 disabled:cursor-not-allowed transition-colors btn-press"
              >
                {loading ? (
          ...
          {significantChanges.length > 0 && (
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
              {significantChanges.map((row: PeriodComparisonRow) => (
                ...
                >
                  <p className="text-xs text-muted truncate mb-1">{row.category}</p>
                  <p
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {(data.rows ?? []).map((row: PeriodComparisonRow) => (
                  <tr
                    key={row.category}
                    className={`border-b border-border/50 hover:bg-overlay transition-colors ${getVarianceBg(row.variancePercent)}`}
                  >
                    <td className="py-2 text-primary">{row.category}</td>
              ...
              {data.summary && (
                <tfoot>
                  <tr className="border-t-2 border-cba-gold/30">
                    ...
                    <td className="pt-3 text-right font-bold font-mono text-primary">
                      {formatCurrency(data.summary.currentTotal)}
                    </td>
                    <td className="pt-3 text-right font-bold font-mono text-secondary">
