...
export function PeriodComparison() {
    ...
    const handleCompare = () => {
        ...
        setLoading(true);
        setError(null);
        reportsApi.comparePeriods(currentStart, currentEnd, priorStart, priorEnd, reportType)
            .then(setData)
    ...
    const significantChanges = (data?.rows ?? [])
        ...
        .sort((a: PeriodComparisonRow, b: PeriodComparisonRow) => Math.abs(b.variancePercent) - Math.abs(a.variancePercent))
        .slice(0, 4);
        ...
        <div className="space-y-6">
            ...
            <div className="neu-raised rounded-2xl p-5">
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    {/* Current Period */}
                    <div>
                        <label className="block text-xs text-zinc-500 font-medium uppercase tracking-wide mb-2">Current Period</label>
                    ...
                    {/* Prior Period */}
                    <div>
                        <label className="block text-xs text-zinc-500 font-medium uppercase tracking-wide mb-2">Prior Period</label>
                        <div className="flex gap-2">
                            <input
                                type="date"
                                value={priorStart}
                    ...
                    {/* Report Type & Action */}
                    <div>
                        <label className="block text-xs text-zinc-500 font-medium uppercase tracking-wide mb-2">Report Type</label>
                        <div className="flex gap-2">
                            <select
                                value={reportType}
                    ...
                    {significantChanges.length > 0 && (
                        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                            {significantChanges.map((row: PeriodComparisonRow) => (
                                <div key={row.category} className={`neu-raised rounded-2xl p-4 border ${Math.abs(row.variancePercent) > 25 ? 'border-red-500/20' : 'border-amber-500/20'}`}>
                                    <p className="text-xs text-zinc-500 truncate mb-1">{row.category}</p>
                                    <p className={`text-lg font-bold font-mono ${getVarianceColor(row.variancePercent)}`}>
                        ...
                        <table className="w-full text-sm">
                            ...
                            <tbody>
                                {(data.rows ?? []).map((row: PeriodComparisonRow) => (
                                    <tr key={row.category} className={`border-b border-white/5 hover:bg-white/5 transition-colors ${getVarianceBg(row.variancePercent)}`}>
                                        <td className="py-2 text-zinc-200">{row.category}</td>
                                        <td className="py-2 text-right font-mono text-zinc-200">{formatCurrency(row.current)}</td>
                                        <td className="py-2 text-right font-mono text-zinc-400">{formatCurrency(row.prior)}</td>
                                        <td className={`py-2 text-right font-mono ${row.varianceDollars >= 0 ? 'text-emerald-400' : 'text-red-400'}`}>
                                            {row.varianceDollars >= 0 ? '+' : ''}{formatCurrency(row.varianceDollars)}
                                        </td>
                                        <td className={`py-2 text-right font-mono font-bold ${getVarianceColor(row.variancePercent)}`}>
                                            {row.variancePercent >= 0 ? '+' : ''}{row.variancePercent.toFixed(1)}%
                                        </td>
                                    </tr>
                                ))}
                            ...
                            {data.summary && (
                                <tfoot>
                                    <tr className="border-t-2 border-[#FFCC00]/30">
                                        ...
                                        <td className="pt-3 text-right font-bold font-mono text-zinc-200">{formatCurrency(data.summary.currentTotal)}</td>
                                        <td className="pt-3 text-right font-bold font-mono text-zinc-400">{formatCurrency(data.summary.priorTotal)}</td>
                                        <td className={`pt-3 text-right font-bold font-mono ${data.summary.netVariance >= 0 ? 'text-emerald-400' : 'text-red-400'}`}>
                                            {data.summary.netVariance >= 0 ? '+' : ''}{formatCurrency(data.summary.netVariance)}
                                        </td>
