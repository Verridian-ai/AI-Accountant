...
function SectionColumn({ title, groups, total, colorClass }: {
    title: string;
    ...
    return (
        <div className="neu-raised rounded-2xl p-5 flex flex-col">
            ...
            <div className="flex-1 space-y-4">
                {groups.map((group) => (
                    <div key={group.label}>
                        <p className="text-xs text-zinc-500 font-medium uppercase tracking-wide mb-2">{group.label}</p>
                        {group.items.length === 0 ? (
                            <p className="text-xs text-zinc-600 italic">No items</p>
                        ) : (
                            group.items.map((item) => (
                                <div key={item.category} className="flex justify-between py-1.5 border-b border-white/5">
...
export function BalanceSheet({ asAtDate }: Props) {
    ...
    useEffect(() => {
        ...
        setLoading(true);
        setError(null);
        reportsApi.fetchBalanceSheet(asAtDate)
            .then(setData)
    ...
    const assetGroups = [
        { label: 'Current Assets', items: Array.isArray(data.assets?.current) ? data.assets.current : [] },
        { label: 'Non-Current Assets', items: Array.isArray(data.assets?.nonCurrent) ? data.assets.nonCurrent : [] },
    ];
    const liabilityGroups = [
        { label: 'Current Liabilities', items: Array.isArray(data.liabilities?.current) ? data.liabilities.current : [] },
        { label: 'Non-Current Liabilities', items: Array.isArray(data.liabilities?.nonCurrent) ? data.liabilities.nonCurrent : [] },
    ];
    const equityGroups = [
        { label: 'Equity', items: Array.isArray(data.equity) ? data.equity : [] },
    ];

    return (
        <div className="space-y-6">
            {/* Balance Check Indicator */}
            <div className={`neu-raised rounded-2xl p-4 flex items-center gap-3 border ${data.isBalanced ? 'border-emerald-500/20' : 'border-red-500/20'}`}>
                {data.isBalanced ? (
                ...
                <span className="ml-auto text-xs text-zinc-500">As at {new Date(asAtDate).toLocaleDateString('en-AU')}</span>
            </div>

            {/* Three-Column Layout */}
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                <SectionColumn title="Assets" groups={assetGroups} total={data.totalAssets} colorClass="text-emerald-400" />
                <SectionColumn title="Liabilities" groups={liabilityGroups} total={data.totalLiabilities} colorClass="text-red-400" />
                <SectionColumn title="Equity" groups={equityGroups} total={data.totalEquity} colorClass="text-[#FFCC00]" />
            </div>
            ...
            <div className="neu-raised rounded-2xl p-5">
                <div className="grid grid-cols-3 gap-4 text-center">
                    <div>
                        ...
                        <p className="text-lg font-bold font-mono text-emerald-400">{formatCurrency(data.totalAssets)}</p>
                    </div>
                    <div>
                        <p className="text-xs text-zinc-500 uppercase tracking-wide mb-1">Total Liabilities</p>
                        <p className="text-lg font-bold font-mono text-red-400">{formatCurrency(data.totalLiabilities)}</p>
                    </div>
                    <div>
                        <p className="text-xs text-zinc-500 uppercase tracking-wide mb-1">Net Equity</p>
                        <p className="text-lg font-bold font-mono text-[#FFCC00]">{formatCurrency(data.totalEquity)}</p>
                    </div>
                </div>
            </div>
        </div>
