...
function FlowSection({ title, inflows, outflows, net }: {
    title: string;
                ...
                <div className="mb-3">
                    ...
                    {inflows.map((item) => (
                        <div key={item.category} className="flex justify-between py-1.5 border-b border-white/5">
                            ...
                            <span className="text-sm font-mono text-emerald-400">+{formatCurrency(item.amount)}</span>
                        </div>
                    ))}
                </div>
            )}
...
export function CashFlow({ periodStart, periodEnd }: Props) {
    ...
    useEffect(() => {
        ...
        setLoading(true);
        setError(null);
        reportsApi.fetchCashFlow(periodStart, periodEnd)
            .then(setData)
    ...
    const segments = [
        ...
        { label: 'Operating', value: data.operating.net, color: data.operating.net >= 0 ? 'bg-emerald-500' : 'bg-red-500' },
        { label: 'Investing', value: data.investing.net, color: data.investing.net >= 0 ? 'bg-emerald-500' : 'bg-red-500' },
        { label: 'Financing', value: data.financing.net, color: data.financing.net >= 0 ? 'bg-emerald-500' : 'bg-red-500' },
    ...
    return (
        <div className="space-y-6">
            {/* Waterfall Chart */}
            <div className="neu-raised rounded-2xl p-5">
                <h3 className="text-sm font-bold uppercase tracking-wider text-zinc-400 mb-4">Cash Flow Waterfall</h3>
                    ...
                    {segments.map((seg) => {
                        ...
                        return (
                            <div key={seg.label} className="flex-1 flex flex-col items-center gap-1">
                                <span className={`text-xs font-mono ${seg.value >= 0 ? 'text-emerald-400' : 'text-red-400'}`}>
            ...
            <div className="neu-raised rounded-2xl p-5">
                <div className="grid grid-cols-3 gap-4 text-center">
                    <div>
                        ...
                        <p className="text-lg font-bold font-mono text-zinc-200">{formatCurrency(data.openingBalance)}</p>
                    </div>
                    <div>
                        <p className="text-xs text-zinc-500 uppercase tracking-wide mb-1">Net Cash Change</p>
                        <p className={`text-lg font-bold font-mono ${data.netCashChange >= 0 ? 'text-emerald-400' : 'text-red-400'}`}>
                    ...
                    <div>
                        ...
                        <p className="text-lg font-bold font-mono text-[#FFCC00]">{formatCurrency(data.closingBalance)}</p>
                    </div>
                </div>
            </div>
        </div>
