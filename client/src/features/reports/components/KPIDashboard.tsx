...
function trendBorder(trend: 'up' | 'down' | 'stable'): string {
    switch (trend) {
        case 'up': return 'border-emerald-500/20';
        case 'down': return 'border-red-500/20';
...
export function KPIDashboard({ period }: Props) {
    ...
    return (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {kpis.map((kpi) => (
                <div key={kpi.metricName} className={`neu-raised rounded-2xl p-5 border ${trendBorder(kpi.trend)} transition-colors hover:bg-white/[0.02]`}>
