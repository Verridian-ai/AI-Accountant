...
export function NodeDetailPanel({ node, onNavigate, onFeedback, onClose }: NodeDetailPanelProps) {
  ...
  return (
    <div className="neu-raised rounded-2xl border border-border/50 overflow-hidden animate-in slide-in-from-right-4 duration-300">
      {/* Header */}
      <div className="p-4 border-b border-border/50 flex items-start justify-between gap-3">
        ...
        <button
          type="button"
          onClick={onClose}
          className="neu-raised-sm p-1.5 rounded-lg text-secondary hover:text-primary shrink-0"
          aria-label="Close"
        >
          <X className="w-4 h-4" />
        </button>
                ...
                <div className="space-y-1">
                  {nodes.map((n) => (
                    <button
                      ...
                      className="w-full text-left text-xs text-primary hover:text-cba-gold py-1 px-2 rounded-lg hover:bg-overlay transition-colors truncate"
                    >
                      {n.label || n.id}
                    </button>
                  ))}
      ...
      <div className="p-4 flex items-center gap-2">
        ...
        <button
          ...
          className="flex-1 flex items-center justify-center gap-1.5 py-2 rounded-xl text-xs font-bold text-cba-gold hover:bg-cba-gold/10 neu-raised-sm transition-colors"
        >
