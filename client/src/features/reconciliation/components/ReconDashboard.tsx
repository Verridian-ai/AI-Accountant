...
const statusConfig: Record<string, { label: string; color: string; icon: typeof Clock }> = {
    ...
    in_progress: { label: 'In Progress', color: 'bg-amber-500/20 text-amber-400 border-amber-500/30', icon: Activity },
...
export function ReconDashboard() {
    ...
    const totalAll = sessions.reduce((sum, s) => sum + (s.matchedCount ?? 0) + (s.unmatchedCount ?? 0), 0);
            ...
            <div className="flex items-center justify-between flex-wrap gap-3">
                <div>
                    ...
                    <p className="text-sm text-zinc-500">
                        Match bank transactions against your ledger
                    </p>
                </div>
