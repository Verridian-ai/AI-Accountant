...
import type { EntityWithDetails, ConsolidationDetailResponse, ConsolidationSnapshotData } from '@/api';
        ...
        <div className="space-y-6">
                ...
                <Select value={selectedParentId} onValueChange={handleParentChange}>
                    ...
                    <SelectContent>
                        {consolidatedParents.length === 0 ? (
                            <SelectItem value="__none" disabled>No consolidated parents</SelectItem>
                        ) : (
            ...
            {detail && (
                <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                        ...
                        <Card className="neu-raised border-0">
                            <CardContent className="p-4">
                                ...
                                <p className={`text-xl font-bold ${detail.consolidatedTotals.netProfit >= 0 ? 'text-emerald-400' : 'text-red-400'}`}>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                        <Card className="neu-raised border-0">
                            <CardContent className="p-4">
                                ...
                                <p className="text-lg font-bold text-zinc-100">{formatMoney(detail.consolidatedTotals.assets)}</p>
                            </CardContent>
                        </Card>
                        <Card className="neu-raised border-0">
                            <CardContent className="p-4">
                                <p className="text-xs text-zinc-500 mb-1">Total Liabilities</p>
                                <p className="text-lg font-bold text-zinc-100">{formatMoney(detail.consolidatedTotals.liabilities)}</p>
                            </CardContent>
                        </Card>
                        <Card className="neu-raised border-0">
                            <CardContent className="p-4">
                                <p className="text-xs text-zinc-500 mb-1">Total Equity</p>
                                <p className="text-lg font-bold text-zinc-100">{formatMoney(detail.consolidatedTotals.equity)}</p>
                            </CardContent>
                        </Card>
                    </div>
                    ...
                    <Card className="neu-raised border-0">
                        <CardHeader>
                            <CardTitle className="text-sm font-bold text-zinc-300">Per-Entity P&L Breakdown</CardTitle>
                        </CardHeader>
                        <CardContent>
                            <div className="overflow-x-auto">
                                <table className="w-full text-sm">
                                    ...
                                    <tbody>
                                        {Object.entries(detail.byEntity).map(([entityId, data]) => {
                                            ...
                                            return (
                                                <tr key={entityId} className="border-b border-white/5">
                                                    ...
                                                    <td className="py-2 px-2 text-right text-emerald-400">{formatMoney(data.revenue)}</td>
                                                    <td className="py-2 px-2 text-right text-red-400">{formatMoney(data.expenses)}</td>
                                                    <td className={`py-2 pl-2 text-right font-bold ${net >= 0 ? 'text-emerald-400' : 'text-red-400'}`}>
                                                        {formatMoney(net)}
                                                    </td>
                                                </tr>
                                            );
                    ...
                    {detail.eliminations.length > 0 && (
                        <Card className="neu-raised border-0">
                            <CardHeader>
                                <CardTitle className="text-sm font-bold text-zinc-300">Elimination Entries</CardTitle>
                            </CardHeader>
                            <CardContent>
                                <div className="space-y-2">
                                    {detail.eliminations.map((elim, idx) => (
                                        <div key={idx} className="flex items-center justify-between py-2 border-b border-white/5 last:border-0">
                                            <span className="text-sm text-zinc-400">{elim.description}</span>
                                            <span className="text-sm font-bold text-blue-400">{formatMoney(elim.amount)}</span>
                                        </div>
                                    ))}
                                </div>
                            </CardContent>
            ...
            {snapshotsLoaded && snapshots.length > 0 && (
                <Card className="neu-raised border-0">
                    ...
                    <CardContent>
                        <div className="space-y-2">
                            {snapshots.map(snap => (
                                <div key={snap.id} className="flex items-center justify-between py-2 border-b border-white/5 last:border-0">
                                    ...
                                    <div className="flex items-center gap-2">
                                        ...
                                        {snap.status === 'draft' && (
                                            ...
                                            >
                                                {finalizing === snap.id ? <Loader2 className="w-3 h-3 animate-spin" /> : <Lock className="w-3 h-3 mr-1" />}
                                                Finalize
                                            </Button>
                                        )}
                                    </div>
