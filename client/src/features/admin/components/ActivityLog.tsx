...
export function ActivityLog() {
    ...
    const statusColor = (s: string) => {
        switch (s) {
            ...
            case 'error': case 'failed': return 'bg-red-500/10 text-red-400';
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="flex items-center gap-3 flex-wrap">
                ...
                <button type="button" onClick={loadData} className="px-3 py-1.5 bg-[#FFCC00] text-[#1a1a2e] rounded-lg text-xs font-bold">Apply</button>
            </div>
            ...
            {showSummary && summary && (
                <div className="rounded-2xl bg-[#16213e] shadow-[6px_6px_12px_#0a0a1a,-6px_-6px_12px_#222244] p-6 space-y-4">
                    <div className="flex items-center justify-between">
                        ...
                        <button type="button" onClick={() => setShowSummary(false)} className="text-xs text-zinc-500 hover:text-white">Close</button>
                    </div>
                            ...
                            <div className="flex items-end gap-px h-20">
                                {summary.dailyCounts.map((d, i) => (
                                    <div key={i} className="flex-1 group relative">
                                        <div className="w-full bg-[#FFCC00]/60 hover:bg-[#FFCC00] rounded-t transition-all"
            ...
            <div className="rounded-2xl bg-[#16213e] shadow-[6px_6px_12px_#0a0a1a,-6px_-6px_12px_#222244] p-6">
                {loading ? (
                    ...
                    <div className="space-y-2">
                        {logs.map((entry, i) => (
                            <div key={entry.id || i} className="flex items-center justify-between py-3 px-4 rounded-xl bg-[#1a1a2e]/50 hover:bg-[#1a1a2e] transition-colors text-xs">
                                <div className="flex items-center gap-3 flex-1 min-w-0">
                                    ...
                                    <div className="min-w-0">
                                        ...
                                        <p className="text-zinc-500 truncate">{entry.resource}{entry.resourceId ? ` #${entry.resourceId}` : ''}</p>
                                    </div>
                                </div>
                                <div className="flex items-center gap-4 shrink-0 ml-4">
                                    {entry.username && <span className="text-zinc-400">{entry.username}</span>}
                                    <span className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase ${statusColor(entry.status)}`}>{entry.status}</span>
                                    <span className="text-zinc-600 w-32 text-right">{new Date(entry.timestamp).toLocaleString()}</span>
                                </div>
                            </div>
                        ))}
                        {logs.length === 0 && <p className="text-sm text-zinc-500 text-center py-8">No activity found</p>}
                    </div>
                )}
                {logs.length >= 30 && (
                    <button type="button" onClick={loadMore} className="mt-4 w-full py-2 text-sm text-[#FFCC00] hover:bg-[#FFCC00]/5 rounded-lg">Load More</button>
                )}
            </div>
        </div>
    );
