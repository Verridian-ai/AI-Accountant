...
import { Bot, Clock, DollarSign, AlertTriangle, ChevronDown, ChevronRight, Hash } from 'lucide-react';
...
export function AgentExecutionDetail({ execution, onClose }: AgentExecutionDetailProps) {
    const [expandedSections, setExpandedSections] = useState<Record<string, boolean>>({ input: false, output: false, tools: true });
            ...
            <div className="relative w-full max-w-3xl max-h-[90vh] overflow-y-auto rounded-2xl bg-[#16213e] shadow-[6px_6px_12px_#0a0a1a,-6px_-6px_12px_#222244] p-6">
                ...
                <div className="flex items-start justify-between mb-6">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h3 className="text-lg font-bold text-white">{execution.agentType?.replace(/_/g, ' ')}</h3>
                    ...
                    <div className="flex items-center gap-3">
                        <span className={`px-3 py-1 rounded-full text-xs font-bold uppercase ${
                            ...
                            execution.status === 'failed' ? 'bg-red-500/10 text-red-400' : 'bg-yellow-500/10 text-yellow-400'
                        }`}>{execution.status}</span>
                        <button type="button" onClick={onClose} className="text-zinc-500 hover:text-white text-xl">&times;</button>
                    </div>
                </div>
                ...
                <div className="grid grid-cols-4 gap-3 mb-6">
                    <div className="rounded-xl bg-[#1a1a2e] p-3 text-center">
                        ...
                        <p className="text-lg font-bold text-white">{(execution.durationMs / 1000).toFixed(2)}s</p>
                    ...
                    <div className="rounded-xl bg-[#1a1a2e] p-3 text-center">
                        ...
                        <p className="text-lg font-bold text-[#FFCC00]">${((execution.costCents || 0) / 100).toFixed(4)}</p>
                ...
                <div className="mb-6">
                    ...
                    <div className="h-3 rounded-full bg-[#1a1a2e] overflow-hidden flex">
                        ...
                        <div className="h-full bg-emerald-500 rounded-r-full" style={{ width: `${100 - inputPct}%` }} />
                    </div>
                ...
                {['input', 'output', 'tools'].map(section => (
                    <div key={section} className="mb-3">
                        ...
                        >
                            ...
                            {section === 'input' ? 'Input Data' : section === 'output' ? 'Output Data' : 'Tool Calls'}
                        </button>
                            ...
                            <div className="rounded-xl bg-[#1a1a2e] p-4 max-h-60 overflow-y-auto">
                                {section === 'tools' && execution.toolCalls ? (
                                    ...
                                    <pre className="text-xs text-zinc-400 whitespace-pre-wrap font-mono">
                                        {JSON.stringify(section === 'input' ? execution.input : execution.output, null, 2) || 'No data'}
                                    </pre>
                                )}
                            </div>
                        )}
                ...
                <p className="text-[10px] text-zinc-600 mt-4">Created: {new Date(execution.createdAt).toLocaleString()}</p>
            </div>
        </div>
    );
}
