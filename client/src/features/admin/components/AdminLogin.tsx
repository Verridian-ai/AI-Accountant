...
export function AdminLogin({ onLogin }: AdminLoginProps) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            onLogin(res.token, { username: res.user?.username || username, role: res.user?.role || 'admin' });
        } catch (err: unknown) {
    ...
    return (
        <div className="min-h-screen bg-[#0a0a1a] flex items-center justify-center p-4">
            <div className="w-full max-w-md">
                ...
                <form onSubmit={handleSubmit} className="rounded-2xl bg-[#16213e] shadow-[6px_6px_12px_#0a0a1a,-6px_-6px_12px_#222244] p-8 space-y-6">
                    {error && (
