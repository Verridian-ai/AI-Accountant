...
import { Server, CheckCircle, AlertTriangle, XCircle, HardDrive, Clock, RefreshCw } from 'lucide-react';
...
export function SystemHealthDashboard() {
    ...
    const [history, setHistory] = useState<Array<{ timestamp: string; status: string; service: string }>>([]);
    ...
    const loadAll = async () => {
        ...
        try {
            ...
            if (d.status === 'fulfilled') setDisk(Array.isArray(d.value) ? d.value : d.value?.disks || []);
    ...
    const statusColor = (s: string) => {
        switch (s) {
            ...
            case 'degraded': return 'border-yellow-500/20 bg-yellow-500/5';
    ...
    if (loading) {
        return <div className="space-y-4"><h2 className="text-2xl font-bold text-white">System Health</h2><div className="animate-pulse h-64 rounded-2xl bg-[#16213e]" /></div>;
    }
    ...
    return (
        <div className="space-y-6">
            <div className="flex items-center justify-between">
                ...
                <button type="button" onClick={loadAll} className="p-2 rounded-xl bg-[#16213e] text-zinc-400 hover:text-[#FFCC00]">
            ...
            <div className={`rounded-2xl border p-6 ${statusColor(health?.status || 'unknown')}`}>
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <p className="text-lg font-bold text-white">System: {(health?.status || 'unknown').toUpperCase()}</p>
                        ...
                        {disk.map((d, i) => (
                            <div key={i}>
                                <div className="flex items-center justify-between text-xs mb-1">
                                    ...
                                    <span className={`font-bold ${d.pct > 90 ? 'text-red-400' : d.pct > 70 ? 'text-yellow-400' : 'text-emerald-400'}`}>
                    ...
                    <div className="space-y-2 max-h-60 overflow-y-auto">
                        {history.slice(0, 50).map((h, i) => (
                            <div key={i} className="flex items-center justify-between py-2 border-b border-white/5 last:border-0 text-xs">
                                <div className="flex items-center gap-2">
                                    <span className={`w-2 h-2 rounded-full ${h.status === 'healthy' ? 'bg-emerald-400' : h.status === 'degraded' ? 'bg-yellow-400' : 'bg-red-400'}`} />
                                    <span className="text-zinc-300">{h.service}</span>
                                </div>
                                <div className="flex items-center gap-3">
                                    <span className={`font-bold ${h.status === 'healthy' ? 'text-emerald-400' : h.status === 'degraded' ? 'text-yellow-400' : 'text-red-400'}`}>
                                        {h.status}
                                    </span>
