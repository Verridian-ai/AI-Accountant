...
import { fetchCogneeAdminDatasets, reindexCogneeDataset, fetchCogneeGraphStats } from '../../../api';
...
export function CogneeManager() {
    ...
    const loadData = async () => {
        ...
        try {
            ...
            if (ds.status === 'fulfilled') setDatasets(Array.isArray(ds.value) ? ds.value : ds.value?.datasets || []);
    ...
    const handleReindexAll = async () => {
        ...
        for (const ds of datasets) {
            try { await reindexCogneeDataset(ds.name); } catch { /* continue */ }
        }
    ...
    if (loading) {
        return <div className="space-y-4"><h2 className="text-2xl font-bold text-white">Cognee Manager</h2><div className="animate-pulse h-64 rounded-2xl bg-[#16213e]" /></div>;
    }
        ...
        <div className="space-y-6">
            <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                    <button type="button" onClick={handleReindexAll} disabled={reindexing !== null}
                    ...
                    <button type="button" onClick={loadData} className="p-2 rounded-xl bg-[#16213e] text-zinc-400 hover:text-[#FFCC00]">
            ...
            {message && (
                <div className={`px-4 py-2 rounded-lg text-sm ${message.includes('failed') ? 'bg-red-500/10 text-red-400' : 'bg-emerald-500/10 text-emerald-400'}`}>
                    {message}
                </div>
            )}
            ...
            {graphStats && (
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                    <div className="rounded-xl bg-[#16213e] p-4 text-center">
                        ...
                        <p className="text-2xl font-bold text-white">{graphStats.totalNodes?.toLocaleString()}</p>
                        <p className="text-[10px] text-zinc-500 uppercase">Nodes</p>
                    </div>
                    <div className="rounded-xl bg-[#16213e] p-4 text-center">
                        <Layers className="w-5 h-5 text-violet-400 mx-auto mb-1" />
                        <p className="text-2xl font-bold text-white">{graphStats.totalEdges?.toLocaleString()}</p>
                        <p className="text-[10px] text-zinc-500 uppercase">Edges</p>
                    </div>
                    <div className="rounded-xl bg-[#16213e] p-4 text-center">
                        <Database className="w-5 h-5 text-emerald-400 mx-auto mb-1" />
                        <p className="text-2xl font-bold text-white">{Object.keys(graphStats.nodeTypes || {}).length}</p>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {datasets.map(ds => (
                    <div key={ds.name} className="rounded-2xl bg-[#16213e] shadow-[6px_6px_12px_#0a0a1a,-6px_-6px_12px_#222244] p-5 border border-white/5 hover:border-[#FFCC00]/10 transition-all">
                        <div className="flex items-center justify-between mb-3">
                            ...
                            {ds.category && (
                                <span className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase ${categoryColors[ds.category] || categoryColors.default}`}>
                                    {ds.category}
                                </span>
                            )}
                        </div>
                        <div className="space-y-1 text-xs text-zinc-400 mb-4">
                            ...
                            {ds.status && (
                                <p>Status: <span className={ds.status === 'ready' ? 'text-emerald-400' : 'text-yellow-400'}>{ds.status}</span></p>
                            )}
                        </div>
                        <button
                            type="button"
