...
interface MetricsData {
    ...
    database: { queryAvgMs: number; connectionPool: number; maxConnections: number; history: MetricPoint[] };
}
...
export function SystemMetricsCharts() {
    ...
    useEffect(() => { loadMetrics(); }, [timeRange]);
    ...
    if (loading) {
        return <div className="space-y-4"><h2 className="text-2xl font-bold text-white">System Metrics</h2><div className="animate-pulse h-64 rounded-2xl bg-[#16213e]" /></div>;
    }
        ...
        <div className="space-y-6">
            <div className="flex items-center justify-between">
                <div>
                    ...
                    <p className="text-sm text-zinc-500">Memory, CPU, API latency, and database performance</p>
                </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                ...
                <div className="rounded-2xl bg-[#16213e] shadow-[6px_6px_12px_#0a0a1a,-6px_-6px_12px_#222244] p-5 flex flex-col items-center">
                    ...
                    <p className="text-[10px] text-zinc-600">
                        {((metrics?.memory?.used ?? 0) / 1073741824).toFixed(1)}GB / {((metrics?.memory?.total ?? 0) / 1073741824).toFixed(1)}GB
                    </p>
                </div>
                ...
                <div className="rounded-2xl bg-[#16213e] shadow-[6px_6px_12px_#0a0a1a,-6px_-6px_12px_#222244] p-5 flex flex-col items-center">
                    ...
                    <p className="text-3xl font-bold text-white">{(metrics?.apiLatency?.avg ?? 0).toFixed(0)}</p>
                ...
                <div className="rounded-2xl bg-[#16213e] shadow-[6px_6px_12px_#0a0a1a,-6px_-6px_12px_#222244] p-5 flex flex-col items-center">
                    ...
                    <p className="text-3xl font-bold text-white">{(metrics?.database?.queryAvgMs ?? 0).toFixed(1)}</p>
