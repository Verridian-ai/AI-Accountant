...
import {
    LayoutDashboard, Bot, DollarSign, Settings, Activity, Server,
    Brain, Search, Users, ScrollText, Flag, BarChart3,
...
export function AdminLayout({ onLogout }: AdminLayoutProps) {
    ...
    useEffect(() => {
        ...
        fetchAdminProfile()
            .then(p => setAdminUser({ username: p.username || 'admin', role: p.role || 'admin' }))
            .catch(() => { /* token might be stale, let them use it until a real call fails */ });
    }, [authenticated]);
    ...
    const renderContent = () => {
        switch (activeSection) {
            ...
            case 'costs': return <AgentCostDashboard />;
            case 'config': return <AgentConfigManager />;
            case 'health': return <SystemHealthDashboard />;
            case 'cognee': return <CogneeManager />;
            case 'search': return <CogneeSearchTester />;
            case 'users': return <UserManager />;
            case 'activity': return <ActivityLog />;
            case 'features': return <FeatureFlagManager />;
            case 'metrics': return <SystemMetricsCharts />;
    ...
    return (
        <div className="min-h-screen bg-[#0a0a1a] flex">
            {/* Sidebar */}
            <aside className={`${sidebarCollapsed ? 'w-16' : 'w-64'} bg-[#0f0f23] border-r border-white/5 flex flex-col transition-all duration-300 shrink-0`}>
                {/* Logo */}
                        ...
                        return (
                            <div key={item.id}>
                                {showGroup && !sidebarCollapsed && (
                                    <p className="text-[10px] font-bold text-zinc-600 uppercase tracking-widest px-3 mt-4 mb-2">{item.group}</p>
                                )}
                                <button
                                    type="button"
                ...
                <div className="p-4 border-t border-white/5 space-y-3">
                    ...
                    {!sidebarCollapsed && adminUser && (
                        <div className="flex items-center gap-2 text-xs text-zinc-500">
                            ...
                            <span className="ml-auto px-1.5 py-0.5 rounded bg-[#FFCC00]/10 text-[#FFCC00] text-[10px] font-bold uppercase">{adminUser.role}</span>
                        </div>
                    )}
                    <button
                        type="button"
