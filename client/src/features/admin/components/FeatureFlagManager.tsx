...
export function FeatureFlagManager() {
    ...
    const [formData, setFormData] = useState({ name: '', description: '', category: 'general', rolloutPercentage: 100 });
    ...
    const updateRollout = async (flag: FeatureFlag, pct: number) => {
        try {
            ...
            setFlags(prev => prev.map(f => f.name === flag.name ? { ...f, rolloutPercentage: pct } : f));
        } catch { /* */ }
    };
    ...
    if (loading) {
        return <div className="space-y-4"><h2 className="text-2xl font-bold text-white">Feature Flags</h2><div className="animate-pulse h-64 rounded-2xl bg-[#16213e]" /></div>;
    }
        ...
        <div className="space-y-6">
            ...
            {Object.entries(grouped).map(([category, categoryFlags]) => (
                <div key={category} className="space-y-3">
                    <h3 className={`text-xs font-bold uppercase tracking-widest ${categoryColors[category] || 'text-zinc-400'}`}>
                        {category}
                    </h3>
                    <div className="space-y-2">
                        {categoryFlags.map(flag => (
                            <div key={flag.name} className="rounded-2xl bg-[#16213e] shadow-[6px_6px_12px_#0a0a1a,-6px_-6px_12px_#222244] p-5">
                                <div className="flex items-center justify-between mb-2">
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div>
                                            ...
                                            {flag.description && <p className="text-xs text-zinc-500">{flag.description}</p>}
                                        </div>
                                    </div>
                                    <span className={`px-2 py-0.5 rounded text-[10px] font-bold ${flag.enabled ? 'bg-emerald-500/10 text-emerald-400' : 'bg-zinc-500/10 text-zinc-500'}`}>
                                        {flag.enabled ? 'ON' : 'OFF'}
                                    </span>
                                </div>
                                {flag.enabled && (
                                    <div className="mt-3 pl-10">
                                        <div className="flex items-center gap-3">
                                            ...
                                            />
                                            <span className="text-xs text-[#FFCC00] font-bold w-10 text-right">{flag.rolloutPercentage ?? 100}%</span>
                                        </div>
                                    </div>
                                )}
                            </div>
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
                    ...
                    <div className="relative w-full max-w-md rounded-2xl bg-[#16213e] shadow-[6px_6px_12px_#0a0a1a,-6px_-6px_12px_#222244] p-6">
                        ...
                        <div className="space-y-4">
                            ...
                            <div>
                                <label className="block text-xs text-zinc-400 mb-1">Rollout: {formData.rolloutPercentage}%</label>
                                <input type="range" min={0} max={100} step={5} value={formData.rolloutPercentage}
