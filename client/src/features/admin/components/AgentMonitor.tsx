...
export function AgentMonitor() {
    const [stats, setStats] = useState<{ totalExecutions: number; successRate: number; avgDuration: number; agentBreakdown: AgentStat[] } | null>(null);
    ...
    const successRate = (s: AgentStat) => s.totalExecutions > 0 ? ((s.successCount / s.totalExecutions) * 100).toFixed(1) : '0.0';
    const rateColor = (rate: number) => rate >= 90 ? 'text-emerald-400' : rate >= 70 ? 'text-yellow-400' : 'text-red-400';

    if (loading) {
        return <div className="space-y-4"><h2 className="text-2xl font-bold text-white">Agent Monitor</h2><div className="animate-pulse h-64 rounded-2xl bg-[#16213e]" /></div>;
    }
    ...
    return (
        <div className="space-y-6">
            <div className="flex items-center justify-between">
                ...
                <button type="button" onClick={loadData} className="p-2 rounded-xl bg-[#16213e] text-zinc-400 hover:text-[#FFCC00] transition-colors">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                ...
                <div className="rounded-2xl bg-[#16213e] shadow-[6px_6px_12px_#0a0a1a,-6px_-6px_12px_#222244] p-5">
                    ...
                    <p className={`text-3xl font-bold mt-1 ${rateColor(stats?.successRate ?? 0)}`}>{(stats?.successRate ?? 0).toFixed(1)}%</p>
                </div>
                <div className="rounded-2xl bg-[#16213e] shadow-[6px_6px_12px_#0a0a1a,-6px_-6px_12px_#222244] p-5">
                    <p className="text-xs text-zinc-500 uppercase tracking-wider">Avg Duration</p>
                    <p className="text-3xl font-bold text-white mt-1">{((stats?.avgDuration ?? 0) / 1000).toFixed(1)}s</p>
                </div>
            </div>
                                ...
                                {stats.agentBreakdown.map(a => {
                                    ...
                                    return (
                                        <tr key={a.agentType}
                                            ...
                                            onClick={() => filterByAgent(selectedAgent === a.agentType ? null : a.agentType)}
                                        >
                                            <td className="py-3 pr-4">
                                                <div className="flex items-center gap-2">
                                                    <Bot className="w-4 h-4 text-[#FFCC00]" />
                                                    <span className={`font-medium ${selectedAgent === a.agentType ? 'text-[#FFCC00]' : 'text-zinc-300'}`}>
                                            ...
                                            <td className="py-3 pr-4 text-zinc-400">{(a.avgDurationMs / 1000).toFixed(1)}s</td>
                                            <td className="py-3 text-[#FFCC00]">${(a.totalCostCents / 100).toFixed(2)}</td>
                                        </tr>
                                    );
                                })}
                            </tbody>
            ...
            <div className="rounded-2xl bg-[#16213e] shadow-[6px_6px_12px_#0a0a1a,-6px_-6px_12px_#222244] p-6">
                <div className="flex items-center justify-between mb-4">
                    <h3 className="text-lg font-bold text-white">
                        Execution History
                        {selectedAgent && <span className="text-sm font-normal text-[#FFCC00] ml-2">({selectedAgent})</span>}
                    </h3>
                    {selectedAgent && (
                        <button type="button" onClick={() => filterByAgent(null)} className="text-xs text-zinc-400 hover:text-white">Clear filter</button>
                    )}
                </div>
                <div className="space-y-2">
                    {executions.length === 0 ? (
                        ...
                        executions.map(ex => (
                            <div key={ex.id} className="flex items-center justify-between py-3 px-4 rounded-xl bg-[#1a1a2e]/50 hover:bg-[#1a1a2e] transition-colors">
                                <div className="flex items-center gap-3">
                                    {ex.status === 'completed' ? <CheckCircle className="w-4 h-4 text-emerald-400" /> :
                                     ex.status === 'failed' ? <XCircle className="w-4 h-4 text-red-400" /> :
                                     <Clock className="w-4 h-4 text-yellow-400" />}
                                    <div>
                                        <p className="text-sm font-medium text-zinc-300">{ex.agentType?.replace(/_/g, ' ')}</p>
                                        {ex.error && <p className="text-xs text-red-400 mt-0.5 truncate max-w-xs">{ex.error}</p>}
                                    </div>
                                </div>
                ...
                {executions.length >= 20 && (
                    <button type="button" onClick={loadMore} className="mt-4 w-full py-2 text-sm text-[#FFCC00] hover:bg-[#FFCC00]/5 rounded-lg transition-colors">
                        Load More
                    </button>
                )}
            </div>
