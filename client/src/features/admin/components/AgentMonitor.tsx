  ...
  if (loading) {
    return (
      <div className="space-y-4">
        <h2 className="text-2xl font-bold text-primary">Agent Monitor</h2>
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-primary">Agent Monitor</h2>
          <p className="text-sm text-muted">AI agent execution tracking and performance</p>
        </div>
        <button
          type="button"
          onClick={() => {
          ...
          className="p-2 rounded-xl bg-[#16213e] text-secondary hover:text-cba-gold transition-colors"
        >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="rounded-2xl bg-[#16213e] shadow-[6px_6px_12px_#0a0a1a,-6px_-6px_12px_#222244] p-5">
          ...
          <p className="text-3xl font-bold text-primary mt-1">{stats?.totalExecutions ?? 0}</p>
        </div>
        <div className="rounded-2xl bg-[#16213e] shadow-[6px_6px_12px_#0a0a1a,-6px_-6px_12px_#222244] p-5">
          <p className="text-xs text-muted uppercase tracking-wider">Success Rate</p>
      ...
      {stats?.agentBreakdown && stats.agentBreakdown.length > 0 && (
        <div className="rounded-2xl bg-[#16213e] shadow-[6px_6px_12px_#0a0a1a,-6px_-6px_12px_#222244] p-6">
          <h3 className="text-lg font-bold text-primary mb-4">Agent Breakdown</h3>
              ...
              <tbody>
                {stats.agentBreakdown.map((a) => {
                  ...
                  return (
                    <tr
                      key={a.agentType}
                      className="border-b border-border/50 hover:bg-[#1a1a2e]/50 cursor-pointer"
                      onClick={() =>
                    ...
                    >
                      <td className="py-3 pr-4">
                        <div className="flex items-center gap-2">
                          ...
                          <span
                            className={`font-medium ${selectedAgent === a.agentType ? 'text-cba-gold' : 'text-primary'}`}
                          >
                            {a.agentType.replace(/_/g, ' ')}
                          </span>
                        </div>
                      </td>
                      <td className="py-3 pr-4 text-secondary">{a.totalExecutions}</td>
                      ...
                      <td className="py-3 text-cba-gold">
      ...
      <div className="rounded-2xl bg-[#16213e] shadow-[6px_6px_12px_#0a0a1a,-6px_-6px_12px_#222244] p-6">
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-lg font-bold text-primary">
            Execution History
            {selectedAgent && (
              <span className="text-sm font-normal text-cba-gold ml-2">({selectedAgent})</span>
            )}
          </h3>
          {selectedAgent && (
            <button
        ...
        <div className="space-y-2">
          {executions.length === 0 ? (
            <p className="text-sm text-muted">No executions found</p>
          ) : (
            executions.map((ex) => (
              <div
                key={ex.id}
              ...
              >
                ...
                <div className="flex items-center gap-6 text-xs text-muted">
                  ...
                  <span className="text-cba-gold">${((ex.costCents || 0) / 100).toFixed(3)}</span>
        ...
        {executions.length >= 20 && (
          <button
            type="button"
            onClick={loadMore}
            className="mt-4 w-full py-2 text-sm text-cba-gold hover:bg-cba-gold/5 rounded-lg transition-colors"
          >
            Load More
          </button>
        )}
