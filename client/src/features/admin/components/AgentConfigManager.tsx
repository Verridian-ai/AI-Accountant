...
export function AgentConfigManager() {
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            setConfigs(prev => prev.map(c => c.agentType === editingAgent.agentType ? editingAgent : c));
    ...
    const toggleEnabled = async (agent: AgentConfig) => {
        try {
            ...
            setConfigs(prev => prev.map(c => c.agentType === agent.agentType ? { ...c, enabled: !c.enabled } : c));
        } catch { /* */ }
    };

    if (loading) {
        return <div className="space-y-4"><h2 className="text-2xl font-bold text-white">Agent Configuration</h2><div className="animate-pulse h-64 rounded-2xl bg-[#16213e]" /></div>;
    }
    ...
    return (
        <div className="space-y-6">
            ...
            {message && (
                <div className={`px-4 py-2 rounded-lg text-sm ${message.includes('saved') ? 'bg-emerald-500/10 text-emerald-400' : 'bg-red-500/10 text-red-400'}`}>
                    {message}
                </div>
            )}
            ...
            <div className="rounded-2xl bg-[#16213e] shadow-[6px_6px_12px_#0a0a1a,-6px_-6px_12px_#222244] p-6 overflow-x-auto">
                            ...
                            <tr key={c.agentType} className="border-b border-white/5 hover:bg-[#1a1a2e]/50">
                                <td className="py-3 pr-4">
                                    <div className="flex items-center gap-2">
                                        ...
                                        <span className="text-zinc-300 font-medium">{c.agentType.replace(/_/g, ' ')}</span>
                                    </div>
                                </td>
                                ...
                                <td className="py-3 pr-4">
                                    <button type="button" onClick={() => toggleEnabled(c)} className="focus:outline-none">
                                        {c.enabled ? <ToggleRight className="w-6 h-6 text-emerald-400" /> : <ToggleLeft className="w-6 h-6 text-zinc-500" />}
                                    </button>
                                </td>
                                <td className="py-3">
                                    <button
                ...
                {configs.length === 0 && <p className="text-sm text-zinc-500 mt-4">No agent configurations found</p>}
            </div>

            {/* Edit Modal */}
            {editingAgent && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
                    <div className="absolute inset-0 bg-black/60 backdrop-blur-sm" onClick={() => setEditingAgent(null)} />
                    <div className="relative w-full max-w-md rounded-2xl bg-[#16213e] shadow-[6px_6px_12px_#0a0a1a,-6px_-6px_12px_#222244] p-6">
                        <div className="flex items-center justify-between mb-6">
                            <h3 className="text-lg font-bold text-white">Edit {editingAgent.agentType.replace(/_/g, ' ')}</h3>
                            <button type="button" onClick={() => setEditingAgent(null)} className="text-zinc-500 hover:text-white"><X className="w-5 h-5" /></button>
                        </div>
                        ...
                        <div className="space-y-4">
                            ...
                            <div>
                                <label className="block text-xs text-zinc-400 mb-1">Max Tokens: {editingAgent.maxTokens}</label>
                                <input
                                    type="range"
                                    min={256}
                                    max={16384}
                            ...
                            <div>
                                <label className="block text-xs text-zinc-400 mb-1">Temperature: {editingAgent.temperature}</label>
                                <input
                                    type="range"
                                    min={0}
                                    max={2}
                            ...
                            <div>
                                ...
                                <input
                                    ...
                                    onChange={e => setEditingAgent({ ...editingAgent, rateLimitPerMinute: parseInt(e.target.value) || 10 })}
                                    className="w-full px-3 py-2 bg-[#1a1a2e] border border-white/10 rounded-lg text-white text-sm focus:outline-none focus:border-[#FFCC00]/50"
                                />
                            </div>
                            <div className="flex items-center justify-between">
                                <label className="text-xs text-zinc-400">Enabled</label>
                                <button type="button" onClick={() => setEditingAgent({ ...editingAgent, enabled: !editingAgent.enabled })}>
                                    {editingAgent.enabled ? <ToggleRight className="w-6 h-6 text-emerald-400" /> : <ToggleLeft className="w-6 h-6 text-zinc-500" />}
                                </button>
                            </div>
                        </div>

                        <div className="flex items-center gap-3 mt-6">
                            <button type="button" onClick={() => setEditingAgent(null)} className="flex-1 py-2 rounded-lg border border-white/10 text-zinc-400 hover:text-white text-sm font-medium">Cancel</button>
                            <button type="button" onClick={handleSave} disabled={saving} className="flex-1 py-2 rounded-lg bg-[#FFCC00] text-[#1a1a2e] font-bold text-sm flex items-center justify-center gap-2 disabled:opacity-50">
