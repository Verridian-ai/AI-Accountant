  ...
  if (!data) {
    return (
      <div className="rounded-2xl bg-[#16213e] p-12 text-center">
        ...
        <button type="button" onClick={onBack} className="mt-4 text-cba-gold text-sm">
          Go back
        </button>
      </div>
    );
    ...
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-sm text-muted">{data.category || 'Uncategorized'} dataset</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="rounded-2xl bg-[#16213e] shadow-[6px_6px_12px_#0a0a1a,-6px_-6px_12px_#222244] p-5 text-center">
          <FileText className="w-5 h-5 text-cba-gold mx-auto mb-2" />
          <p className="text-2xl font-bold text-primary">{data.documentCount ?? 0}</p>
          <p className="text-xs text-muted">Documents</p>
        </div>
        <div className="rounded-2xl bg-[#16213e] shadow-[6px_6px_12px_#0a0a1a,-6px_-6px_12px_#222244] p-5 text-center">
          <Database className="w-5 h-5 text-emerald-400 mx-auto mb-2" />
          <p className="text-2xl font-bold text-primary">{data.entityCount ?? 0}</p>
          <p className="text-xs text-muted">Entities</p>
        </div>
        <div className="rounded-2xl bg-[#16213e] shadow-[6px_6px_12px_#0a0a1a,-6px_-6px_12px_#222244] p-5 text-center">
          <Layers className="w-5 h-5 text-violet-400 mx-auto mb-2" />
          <p className="text-2xl font-bold text-primary">{data.relationshipCount ?? 0}</p>
      ...
      {data.entityTypes && Object.keys(data.entityTypes).length > 0 && (
        <div className="rounded-2xl bg-[#16213e] shadow-[6px_6px_12px_#0a0a1a,-6px_-6px_12px_#222244] p-6">
          <h3 className="text-lg font-bold text-primary mb-4">Entity Types</h3>
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
            {Object.entries(data.entityTypes).map(([type, count]) => (
              ...
              >
                ...
                <span className="text-xs font-bold text-cba-gold">{count}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      {data.relationshipTypes && Object.keys(data.relationshipTypes).length > 0 && (
        <div className="rounded-2xl bg-[#16213e] shadow-[6px_6px_12px_#0a0a1a,-6px_-6px_12px_#222244] p-6">
          <h3 className="text-lg font-bold text-primary mb-4">Relationship Types</h3>
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
      ...
      {data.documents && data.documents.length > 0 && (
        <div className="rounded-2xl bg-[#16213e] shadow-[6px_6px_12px_#0a0a1a,-6px_-6px_12px_#222244] p-6">
          <h3 className="text-lg font-bold text-primary mb-4">Documents</h3>
          <div className="space-y-2">
            {data.documents.map((doc) => (
              ...
              >
                ...
                <span className="text-muted">{new Date(doc.addedAt).toLocaleDateString()}</span>
              </div>
            ))}
          </div>
        </div>
