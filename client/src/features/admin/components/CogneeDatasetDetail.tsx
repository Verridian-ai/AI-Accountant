    ...
    if (!data) {
        return (
            <div className="rounded-2xl bg-[#16213e] p-12 text-center">
                ...
                <button type="button" onClick={onBack} className="mt-4 text-[#FFCC00] text-sm">Go back</button>
            </div>
        );
    }
    ...
    return (
        <div className="space-y-6">
            <div className="flex items-center gap-3">
                <button type="button" onClick={onBack} className="p-2 rounded-xl bg-[#16213e] text-zinc-400 hover:text-[#FFCC00]">
                ...
                <div className="rounded-2xl bg-[#16213e] shadow-[6px_6px_12px_#0a0a1a,-6px_-6px_12px_#222244] p-6">
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                        {Object.entries(data.entityTypes).map(([type, count]) => (
                            <div key={type} className="flex items-center justify-between py-2 px-3 rounded-lg bg-[#1a1a2e]">
                ...
                <div className="rounded-2xl bg-[#16213e] shadow-[6px_6px_12px_#0a0a1a,-6px_-6px_12px_#222244] p-6">
                    ...
                    <div className="space-y-2">
                        {data.documents.map(doc => (
                            <div key={doc.id} className="flex items-center justify-between py-2 px-3 rounded-lg bg-[#1a1a2e] text-xs">
            ...
            {data.lastIndexed && (
                <p className="text-[10px] text-zinc-600">Last indexed: {new Date(data.lastIndexed).toLocaleString()}</p>
            )}
        </div>
    );
}
