        ...
        <div className="space-y-6">
            ...
            <div className="rounded-2xl bg-[#16213e] shadow-[6px_6px_12px_#0a0a1a,-6px_-6px_12px_#222244] p-6 space-y-4">
                ...
                <div>
                    <label className="block text-xs text-zinc-400 mb-2">
                        <Database className="w-3 h-3 inline mr-1" /> Datasets {selectedDatasets.length > 0 && `(${selectedDatasets.length})`}
                    </label>
                    <div className="flex flex-wrap gap-2">
                        ...
                        {datasets.length === 0 && <p className="text-xs text-zinc-500">No datasets available</p>}
                    </div>
                </div>
            ...
            {results.length > 0 && (
                <div className="space-y-4">
                    {results.map((r, ri) => (
                        <div key={ri} className="rounded-2xl bg-[#16213e] shadow-[6px_6px_12px_#0a0a1a,-6px_-6px_12px_#222244] p-6">
                            ...
                            <div className="space-y-3">
                                {r.results.map((item, ii) => (
                                    <div key={ii} className="py-3 px-4 rounded-xl bg-[#1a1a2e] border-l-2 border-[#FFCC00]/20">
                                        ...
                                        {item.score !== undefined && (
                                            <div className="mt-2 flex items-center gap-2">
                                                <div className="h-1.5 flex-1 rounded-full bg-[#0a0a1a] overflow-hidden">
                                                    <div className="h-full bg-[#FFCC00] rounded-full" style={{ width: `${item.score * 100}%` }} />
                                                </div>
                                                <span className="text-[10px] text-zinc-500">{(item.score * 100).toFixed(1)}%</span>
                                            </div>
                                        )}
                                    </div>
                                ))}
