...
export function ForecastScenarios({ onCompare }: ForecastScenariosProps) {
    ...
    const [scenarioType, setScenarioType] = useState<'optimistic' | 'realistic' | 'pessimistic' | 'custom'>('realistic');
                            ...
                            <div className="p-4 flex items-center gap-3">
                                ...
                                <div className="flex-1 min-w-0 cursor-pointer" onClick={() => handleExpand(s.id)}>
                                    <div className="flex items-center gap-2 mb-1">
                                        ...
                                        <span className={`px-2 py-0.5 rounded-full text-[10px] font-bold uppercase ${typeColors[s.scenarioType] ?? typeColors.custom}`}>
                                            {s.scenarioType}
                                        </span>
                                    </div>
                                ...
                                <button onClick={() => handleExpand(s.id)} className="text-zinc-500 hover:text-zinc-300 p-1">
                                    ...
                                    <table className="w-full text-xs">
                                        <thead>
                                            <tr className="border-b border-white/5">
                                                <th className="text-left px-4 py-2 text-zinc-500 font-bold uppercase tracking-wider">Period</th>
                                                <th className="text-left px-4 py-2 text-zinc-500 font-bold uppercase tracking-wider">Category</th>
                                                <th className="text-right px-4 py-2 text-zinc-500 font-bold uppercase tracking-wider">Amount</th>
                                                <th className="text-right px-4 py-2 text-zinc-500 font-bold uppercase tracking-wider">Confidence Range</th>
                                                <th className="text-left px-4 py-2 text-zinc-500 font-bold uppercase tracking-wider">Method</th>
                                            </tr>
                                        </thead>
                                        <tbody>
                                            {(periods[s.id] ?? []).map((p, i) => (
                                                <tr key={i} className="border-b border-white/5 hover:bg-white/[0.02]">
                                                    ...
                                                    <td className="px-4 py-2 text-right text-[#FFCC00] font-mono">{fmt(p.forecastAmount)}</td>
