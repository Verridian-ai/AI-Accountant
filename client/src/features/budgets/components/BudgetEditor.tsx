                            ...
                            <span key={s} className="flex items-center">
                                <button
                                    ...
                                    className={`px-3 py-1.5 rounded-lg text-xs font-bold uppercase tracking-wider transition-all ${
                                        budget.status === s
                                            ? s === 'draft' ? 'bg-zinc-700 text-zinc-200'
                    ...
                    <table className="w-full text-sm">
                        <thead>
                            <tr className="border-b border-white/5">
                                ...
                                <th className="text-left px-4 py-3 text-zinc-500 font-bold uppercase text-xs tracking-wider">Notes</th>
                                <th className="text-center px-4 py-3 text-zinc-500 font-bold uppercase text-xs tracking-wider w-16">Actions</th>
                            </tr>
                        </thead>
                        <tbody>
                            {lines.map(line => (
                                <tr key={line.id} className="border-b border-white/5 hover:bg-white/[0.02] transition-colors">
                                    ...
                                    <td className="px-4 py-3 text-right">
                                        {editingCell === `${line.id}-amount` ? (
                                            <input
                                                ...
                                                onKeyDown={e => e.key === 'Enter' && handleSaveCell(line.id, 'budgetedAmount')}
                                            ...
                                            >
                                                ${((line.budgetedAmount ?? 0) / 100).toLocaleString('en-AU', { minimumFractionDigits: 2 })}
                                            </button>
                                        )}
                                    </td>
                        ...
                        <tfoot>
                            <tr className="border-t border-[#FFCC00]/20 bg-[#FFCC00]/5">
                                <td colSpan={2} className="px-4 py-3 text-zinc-300 font-bold">Total</td>
