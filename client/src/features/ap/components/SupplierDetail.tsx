    ...
    return (
      <div className="space-y-6">
        ...
        <div className="neu-raised rounded-2xl p-8 text-center">
          <p className="text-zinc-400">Supplier not found</p>
        </div>
      </div>
    );
  }
    ...
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="neu-raised rounded-2xl p-6">
        <div className="flex items-start gap-4">
          <div className="h-12 w-12 rounded-xl bg-[#FFCC00]/10 flex items-center justify-center shrink-0">
            <Building2 className="h-6 w-6 text-[#FFCC00]" />
          </div>
      ...
      <div className="neu-raised rounded-2xl p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div>
            <p className="text-xs text-zinc-500 mb-1">BSB</p>
          ...
          <div>
            ...
            <p className="text-sm text-white">{supplier.bankAccountName ?? '—'}</p>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="neu-raised rounded-2xl p-5 text-center">
          ...
          <p className="text-xl font-bold text-white">{formatCurrency(supplier.totalSpent)}</p>
        </div>
        <div className="neu-raised rounded-2xl p-5 text-center">
      ...
      <div className="neu-raised rounded-2xl p-6">
        ...
        {bills.length === 0 ? (
          <p className="text-sm text-zinc-500 text-center py-4">No bills found</p>
        ) : (
          <table className="w-full">
            <thead>
              <tr className="border-b border-white/5">
                <th className="text-left px-3 py-2 text-xs font-semibold text-zinc-400">Bill #</th>
                <th className="text-left px-3 py-2 text-xs font-semibold text-zinc-400">Date</th>
                <th className="text-left px-3 py-2 text-xs font-semibold text-zinc-400">Status</th>
                <th className="text-right px-3 py-2 text-xs font-semibold text-zinc-400">Total</th>
              </tr>
            </thead>
            <tbody>
              {bills.map((bill) => (
                <tr key={bill.id} className="border-b border-white/5">
                  <td className="px-3 py-2 text-sm text-white font-mono">{bill.billNumber}</td>
                  <td className="px-3 py-2 text-sm text-zinc-400">{bill.issueDate}</td>
