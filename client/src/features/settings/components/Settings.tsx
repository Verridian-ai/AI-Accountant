...
import { X, Save, Info, Loader2, CheckCircle2, Sparkles, Cpu, Zap, Activity, Brain, Edit2 } from 'lucide-react';
                        ...
                        <>
                            ...
                            <div className="space-y-8">
                                <SettingSection
                                    ...
                                    onChange={(val) => setSettings(s => s ? { ...s, modelParsingText: val } : null)}
                                    models={chatModels}
                                ...
                                <SettingSection
                                    ...
                                    onChange={(val) => setSettings(s => s ? { ...s, modelParsingVision: val } : null)}
                                    models={visionModels}
                                    onHover={setHoveredModel}
                                ...
                                <SettingSection
                                    ...
                                    onChange={(val) => setSettings(s => s ? { ...s, modelCategorization: val } : null)}
                                    models={chatModels}
                                    onHover={setHoveredModel}
                            ...
                            {hoveredModel && (
                                <div className="p-5 neu-inset rounded-2xl animate-in fade-in slide-in-from-bottom-2 border border-white/5">
                                    <div className="flex items-start gap-4">
                                        ...
                                        <div>
                                            <h4 className="font-black text-zinc-100 uppercase tracking-wide text-sm">{hoveredModel.name}</h4>
                                            <p className="text-zinc-500 mt-1 text-sm leading-relaxed">{hoveredModel.description}</p>
...
function SettingSection({ label, description, value, onChange, models, onHover, icon }: SettingSectionProps) {
    return (
        <div className="group space-y-3">
            <div className="flex items-center gap-3">
                ...
                <div className="flex flex-col">
                    <label className="text-xs font-black text-zinc-200 uppercase tracking-widest">{label}</label>
                    <span className="text-[10px] font-bold text-zinc-600 uppercase tracking-tight">{description}</span>
                </div>
            </div>
