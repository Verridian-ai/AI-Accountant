...
import { Home, Car, Banknote, ArrowRightLeft, TrendingUp, Calculator, BarChart3, Loader2 } from 'lucide-react';
...
export function LoanDashboard() {
    ...
    return (
        <div className="space-y-4 sm:space-y-6">
            <div>
                <h2 className="text-lg sm:text-2xl font-bold tracking-tight text-gradient-gold">Loan Calculators</h2>
                <p className="text-xs sm:text-sm text-zinc-500">
                    Australian loan comparison tools with CDR market rates, offset, refinance, and APRA-compliant capacity estimates
                </p>
            </div>
                        ...
                        <div className="flex items-center justify-between flex-wrap gap-4">
                            ...
                            <div className="grid grid-cols-2 sm:flex sm:items-center gap-3 sm:gap-6 flex-wrap">
                                <div className="text-center">
                                    <p className="text-[10px] uppercase tracking-wide text-zinc-500">Lowest Variable</p>
                                    <p className="text-base sm:text-lg font-bold text-emerald-400">{(marketRates.lowest_variable * 100).toFixed(2)}%</p>
                                </div>
                                <div className="text-center">
                                    <p className="text-[10px] uppercase tracking-wide text-zinc-500">Lowest 2yr Fixed</p>
                                    <p className="text-base sm:text-lg font-bold text-blue-400">{(marketRates.lowest_2yr_fixed * 100).toFixed(2)}%</p>
                                </div>
                                <div className="text-center">
                                    <p className="text-[10px] uppercase tracking-wide text-zinc-500">Avg Variable</p>
                                    <p className="text-base sm:text-lg font-bold text-zinc-300">{(marketRates.average_variable * 100).toFixed(2)}%</p>
                                </div>
                                <div className="text-center">
                                    <p className="text-[10px] uppercase tracking-wide text-zinc-500">Products</p>
                                    <p className="text-base sm:text-lg font-bold text-zinc-300">{marketRates.total_products}</p>
                                </div>
                            </div>
