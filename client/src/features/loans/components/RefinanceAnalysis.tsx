...
import { Loader2, ArrowRightLeft, TrendingDown, CheckCircle, AlertTriangle, MinusCircle } from 'lucide-react';
...
const recommendationConfig = {
    switch: { label: 'Switch', color: 'bg-emerald-500/10 text-emerald-400 border-emerald-500/20', icon: CheckCircle },
    negotiate: { label: 'Negotiate', color: 'bg-yellow-500/10 text-yellow-400 border-yellow-500/20', icon: AlertTriangle },
    stay: { label: 'Stay', color: 'bg-zinc-500/10 text-zinc-400 border-zinc-500/20', icon: MinusCircle },
};
                ...
                <CardContent className="space-y-4">
                    ...
                    <div className="grid gap-4 md:grid-cols-3">
                        ...
                        <div className="space-y-2">
                            ...
                            <Select value={input.loanPurpose} onValueChange={v => updateField('loanPurpose', v)}>
                    ...
                    <Button onClick={handleAnalyze} disabled={loading} className="bg-[#FFCC00] text-[#0a0a0f] hover:bg-[#e6b800]">
                        ...
                        <Card className="neu-raised border-white/5">
                            ...
                            <CardContent>
                                <div className="space-y-3">
                                    {result.alternatives.filter(a => a.feature_match.length > 0).map((alt, i) => (
                                        <div key={i} className="flex items-center gap-3 flex-wrap">
                                            <span className="text-sm font-medium min-w-[120px]">{alt.lender}</span>
                                            <div className="flex flex-wrap gap-1">
                                                {alt.feature_match.map(f => (
                                                    <Badge key={f} className="bg-emerald-500/10 text-emerald-400 border-emerald-500/20 text-xs">
                                                        {f}
                                                    </Badge>
                                                ))}
                                            </div>
