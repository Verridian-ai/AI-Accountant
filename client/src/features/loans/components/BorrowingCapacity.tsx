...
function dtiBadge(ratio: number): { label: string; color: string } {
    if (ratio < 6) return { label: 'Healthy', color: 'bg-emerald-500/10 text-emerald-400 border-emerald-500/20' };
    if (ratio <= 8) return { label: 'Caution', color: 'bg-yellow-500/10 text-yellow-400 border-yellow-500/20' };
                ...
                <CardContent className="space-y-4">
                    <div className="grid gap-4 md:grid-cols-3">
                        ...
                        <div className="space-y-2">
                            ...
                            <Input type="number" value={livingExpenses} onChange={e => setLivingExpenses(e.target.value)} />
                        </div>
                    </div>
                    <div className="grid gap-4 md:grid-cols-3">
                        <div className="space-y-2">
                    ...
                    <div className="space-y-3">
                        ...
                        {debts.map(debt => (
                            <div key={debt.id} className="grid gap-3 md:grid-cols-4 items-end p-3 rounded-lg bg-white/5">
                                ...
                                <Button variant="ghost" size="sm" onClick={() => removeDebt(debt.id)} className="text-red-400 hover:text-red-300">
                ...
                <>
                        ...
                        <Card className="neu-raised border-white/5">
                            <CardContent className="pt-6">
                                ...
                                <p className="text-xs text-zinc-500 mt-1">
                                    {result.dti_ratio < 6 ? 'Within recommended limits' : result.dti_ratio <= 8 ? 'Approaching APRA limits' : 'Exceeds recommended limits'}
                                </p>
                            </CardContent>
                        </Card>
                        <Card className="neu-raised border-white/5">
                    ...
                    <div className="p-4 rounded-lg bg-white/5 border border-white/10">
                        <p className="text-xs text-zinc-500 leading-relaxed">
                            <strong className="text-zinc-400">Disclaimer:</strong> This is an estimate only and does not constitute
                            a loan offer or pre-approval. Actual borrowing capacity depends on individual lender criteria, credit
                            history, and APRA regulatory requirements. A 3% serviceability buffer rate is applied per APRA guidelines.
                            Consult a licensed mortgage broker or financial adviser for personalised advice.
                        </p>
                    </div>
                </>
            )}
