...
export function RateScenarios() {
    ...
    const sourceLabel = (source: string) => {
        switch (source) {
            case 'cdr': return { text: 'CDR', color: 'bg-[#FFCC00]/10 text-[#FFCC00] border-[#FFCC00]/20' };
            case 'stress': return { text: 'Stress', color: 'bg-red-500/10 text-red-400 border-red-500/20' };
            default: return { text: 'Custom', color: 'bg-blue-500/10 text-blue-400 border-blue-500/20' };
        }
    };

    // Find the best (lowest monthly) and worst scenarios for highlighting
    const bestIdx = result?.scenarios.reduce((best, s, i) =>
        s.monthly_repayment < (result.scenarios[best]?.monthly_repayment ?? Infinity) ? i : best, 0) ?? -1;
    const worstIdx = result?.scenarios.reduce((worst, s, i) =>
        s.monthly_repayment > (result.scenarios[worst]?.monthly_repayment ?? -Infinity) ? i : worst, 0) ?? -1;
                ...
                <CardContent className="space-y-4">
                    ...
                    <div className="space-y-3">
                        ...
                        {customScenarios.map(scenario => (
                            <div key={scenario.id} className="grid gap-3 md:grid-cols-3 items-end p-3 rounded-lg bg-white/5">
                                ...
                                <Button variant="ghost" size="sm" onClick={() => removeCustomScenario(scenario.id)} className="text-red-400 hover:text-red-300">
                    ...
                    <Button onClick={handleGenerate} disabled={loading} className="bg-[#FFCC00] text-[#0a0a0f] hover:bg-[#e6b800]">
                ...
                <>
                    ...
                    <div className="grid gap-4 md:grid-cols-3">
                        ...
                        <Card className="neu-raised border-[#FFCC00]/10">
                            <CardContent className="pt-6 text-center">
                                ...
                                <p className="text-xl font-bold text-[#FFCC00]">{(result.base_rate * 100).toFixed(2)}%</p>
                            </CardContent>
                        </Card>
                    </div>
                                        ...
                                        {result.scenarios.map((s, i) => {
                                            ...
                                            return (
                                                ...
                                                >
                                                    ...
                                                    <td className={`text-right py-3 px-2 ${
                                                        s.difference_monthly < 0 ? 'text-emerald-400' :
                                                        s.difference_monthly > 0 ? 'text-red-400' : 'text-zinc-400'
                                                    }`}>
                                                        {s.difference_monthly !== 0 && (s.difference_monthly > 0 ? '+' : '')}
                                                        {s.difference_monthly === 0 ? '—' : formatCurrency(s.difference_monthly / 100)}
                                                    </td>
                                                    <td className={`text-right py-3 pl-2 ${
                                                        s.difference_total < 0 ? 'text-emerald-400' :
                                                        s.difference_total > 0 ? 'text-red-400' : 'text-zinc-400'
                                                    }`}>
                                                        {s.difference_total !== 0 && (s.difference_total > 0 ? '+' : '')}
                                                        {s.difference_total === 0 ? '—' : formatCurrency(s.difference_total / 100)}
                                                    </td>
                                                </tr>
                                            );
                                        })}
                    ...
                    <Card className="neu-raised border-white/5">
                        ...
                        <CardContent>
                            <div className="space-y-3">
                                {result.scenarios.map((s, i) => {
                                    const maxRepayment = Math.max(...result.scenarios.map(x => x.monthly_repayment));
                                    ...
                                    return (
                                        <div key={i} className="space-y-1">
                                            <div className="flex items-center justify-between text-xs">
                                                ...
                                                <span className="font-medium">{formatCurrency(s.monthly_repayment / 100)}/mo</span>
                                            </div>
                                            <div className="h-3 rounded-full bg-white/5 overflow-hidden">
                                                <div
                                                    className={`h-full rounded-full transition-all ${
                                                        isBest ? 'bg-emerald-500' : s.source === 'stress' ? 'bg-red-500' : 'bg-[#FFCC00]'
                                                    }`}
                                                    style={{ width: `${pct}%` }}
                                                />
                                            </div>
