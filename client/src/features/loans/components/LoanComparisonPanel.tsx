        ...
        <div className="space-y-6">
            <Card className="neu-raised border-white/5">
                <CardHeader>
                    <CardTitle className="flex items-center gap-2">
                        <TrendingDown className="w-5 h-5 text-[#FFCC00]" />
                        Refinance Savings
                    </CardTitle>
                ...
                <CardContent className="space-y-4">
                    ...
                    <div className="grid gap-4 md:grid-cols-3">
                        ...
                        <div className="space-y-2">
                            ...
                            <Input type="number" value={switchingCosts} onChange={e => setSwitchingCosts(e.target.value)} />
                        </div>
                    </div>
                    <Button onClick={handleCalculate} disabled={loading} className="bg-[#FFCC00] text-[#0a0a0f] hover:bg-[#e6b800]">
            ...
            {result && (
                <div className="grid gap-4 md:grid-cols-2">
                    ...
                    <Card className="md:col-span-2 neu-raised border-[#FFCC00]/10">
                        <CardContent className="pt-6">
                            <div className="flex items-center justify-around flex-wrap gap-4">
                                <div className="text-center">
                                    <p className="text-sm text-zinc-400">Monthly Saving</p>
                                    <p className="text-2xl font-bold text-emerald-400">{formatCurrency(result.monthlySaving)}</p>
                                </div>
                                <div className="text-center">
                                    <p className="text-sm text-zinc-400">Break Even</p>
                                    <p className="text-2xl font-bold text-[#FFCC00]">{result.breakEvenMonths} months</p>
                                </div>
                                <div className="text-center">
                                    <p className="text-sm text-zinc-400">Total Savings</p>
                                    <p className={`text-2xl font-bold ${result.totalSavings > 0 ? 'text-emerald-400' : 'text-red-400'}`}>
                ...
                <Card className="neu-raised border-[#FFCC00]/10">
                    <CardContent className="pt-6 space-y-4">
                        <div className="text-center mb-6">
                            <p className="text-sm text-zinc-400">Maximum Borrowing</p>
                            <p className="text-4xl font-bold text-[#FFCC00]">{formatCurrency(result.maxBorrowing)}</p>
                        </div>
                        <div className="grid gap-3 md:grid-cols-2">
                            <div className="flex justify-between p-3 rounded-lg bg-white/5">
                                <span className="text-zinc-400">Assessment Rate</span>
                                <span className="font-medium">{formatPercent(result.assessmentRate)}</span>
                            </div>
                            <div className="flex justify-between p-3 rounded-lg bg-white/5">
                                <span className="text-zinc-400">Monthly Repayment</span>
                                <span className="font-medium">{formatCurrency(result.monthlyRepayment)}</span>
                            </div>
                            <div className="flex justify-between p-3 rounded-lg bg-white/5">
                                <span className="text-zinc-400">Net Monthly Income</span>
                                <span className="font-medium">{formatCurrency(result.netMonthlyIncome)}</span>
                            </div>
                            <div className="flex justify-between p-3 rounded-lg bg-white/5">
                                <span className="text-zinc-400">Surplus Income</span>
                                <span className={`font-medium ${result.surplusIncome >= 0 ? 'text-emerald-400' : 'text-red-400'}`}>
                                    {formatCurrency(result.surplusIncome)}
                                </span>
                            </div>
                            <div className="flex justify-between p-3 rounded-lg bg-white/5">
                                <span className="text-zinc-400">Debt Service Ratio</span>
                                <span className="font-medium">{formatPercent(result.dsr)}</span>
                            </div>
                            <div className="flex justify-between p-3 rounded-lg bg-white/5">
                                <span className="text-zinc-400">HEM Floor</span>
