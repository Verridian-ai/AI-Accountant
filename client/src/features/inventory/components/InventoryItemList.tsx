...
export function InventoryItemList() {
    ...
    const [formData, setFormData] = useState({
        sku: '', name: '', category: '', unit: 'each',
        costPriceCents: 0, salePriceCents: 0, reorderPoint: 10,
    });
    ...
    const handleSubmit = async () => {
        try {
            ...
            setShowForm(false);
            setEditItem(null);
            setFormData({ sku: '', name: '', category: '', unit: 'each', costPriceCents: 0, salePriceCents: 0, reorderPoint: 10 });
            loadItems();
        } catch (err) {
    ...
    const openEdit = (item: InventoryItem) => {
        ...
        setFormData({
            ...
            costPriceCents: item.costPriceCents, salePriceCents: item.salePriceCents,
    ...
    return (
        <div className="space-y-4">
            ...
            <div className="flex flex-col sm:flex-row gap-3">
                ...
                <button
                    onClick={() => { setEditItem(null); setFormData({ sku: '', name: '', category: '', unit: 'each', costPriceCents: 0, salePriceCents: 0, reorderPoint: 10 }); setShowForm(true); }}
            ...
            {showForm && (
                <div className="neu-raised rounded-2xl p-6 space-y-4 border border-[#FFCC00]/10">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                        ...
                        <div>
                            <label className="text-xs text-zinc-500 font-semibold uppercase">Cost Price (cents)</label>
                            <input type="number" value={formData.costPriceCents} onChange={e => setFormData(p => ({ ...p, costPriceCents: Number(e.target.value) }))}
                                className="w-full mt-1 px-3 py-2 rounded-lg neu-inset bg-transparent text-zinc-100 text-sm" />
                        </div>
                        <div>
                            <label className="text-xs text-zinc-500 font-semibold uppercase">Sale Price (cents)</label>
                            <input type="number" value={formData.salePriceCents} onChange={e => setFormData(p => ({ ...p, salePriceCents: Number(e.target.value) }))}
                                className="w-full mt-1 px-3 py-2 rounded-lg neu-inset bg-transparent text-zinc-100 text-sm" />
                        </div>
                        <div>
                            <label className="text-xs text-zinc-500 font-semibold uppercase">Reorder Point</label>
                            <input type="number" value={formData.reorderPoint} onChange={e => setFormData(p => ({ ...p, reorderPoint: Number(e.target.value) }))}
                                className="w-full mt-1 px-3 py-2 rounded-lg neu-inset bg-transparent text-zinc-100 text-sm" />
                        </div>
                    </div>
                    <div className="flex gap-3">
                        <button onClick={handleSubmit} className="px-5 py-2 rounded-xl bg-[#FFCC00] text-[#0a0a0f] font-bold text-sm">
                            {editItem ? 'Update' : 'Create'}
                        </button>
                        <button onClick={() => { setShowForm(false); setEditItem(null); }} className="px-5 py-2 rounded-xl neu-raised text-zinc-400 font-bold text-sm">
                            Cancel
                        </button>
                    </div>
                </div>
                        ...
                        <table className="w-full text-sm">
                            <thead>
                                <tr className="border-b border-white/5">
                                    <th className="text-left px-4 py-3 text-xs font-bold text-zinc-500 uppercase">SKU</th>
                                    <th className="text-left px-4 py-3 text-xs font-bold text-zinc-500 uppercase">Name</th>
                                    <th className="text-left px-4 py-3 text-xs font-bold text-zinc-500 uppercase">Category</th>
                                    <th className="text-left px-4 py-3 text-xs font-bold text-zinc-500 uppercase">Unit</th>
                                    <th className="text-right px-4 py-3 text-xs font-bold text-zinc-500 uppercase">Cost</th>
                                    <th className="text-right px-4 py-3 text-xs font-bold text-zinc-500 uppercase">Sale Price</th>
                                    <th className="text-right px-4 py-3 text-xs font-bold text-zinc-500 uppercase">Stock</th>
                                    <th className="text-right px-4 py-3 text-xs font-bold text-zinc-500 uppercase">Reorder</th>
                                    <th className="text-center px-4 py-3 text-xs font-bold text-zinc-500 uppercase">Status</th>
                                    <th className="text-right px-4 py-3 text-xs font-bold text-zinc-500 uppercase">Actions</th>
                                </tr>
                            </thead>
                            <tbody>
                                {items.map(item => (
                                    <tr key={item.id} className={cn(
                                        ...
                                        <td className="px-4 py-3 text-right text-zinc-300">{formatAUD(item.costPriceCents)}</td>
                                        <td className="px-4 py-3 text-right text-zinc-300">{formatAUD(item.salePriceCents)}</td>
                                        <td className={cn(
                                            "px-4 py-3 text-right font-bold",
                                            item.currentStockQty < item.reorderPoint ? "text-amber-400" : "text-emerald-400"
                                        )}>
                                        ...
                                        <td className="px-4 py-3 text-center">
                                            <span className={cn(
                                                ...
                                                item.isActive ? "bg-emerald-500/10 text-emerald-400" : "bg-zinc-500/10 text-zinc-500"
                                            )}>
                                        ...
                                        <td className="px-4 py-3 text-right">
                                            <div className="flex items-center justify-end gap-1">
                                                <button onClick={() => openEdit(item)} className="p-1.5 rounded-lg hover:bg-white/5 text-zinc-500 hover:text-[#FFCC00]">
                                                    <Edit2 className="w-3.5 h-3.5" />
                                                </button>
                                                <button onClick={() => handleDeactivate(item.id)} className="p-1.5 rounded-lg hover:bg-white/5 text-zinc-500 hover:text-red-400">
