...
export function ValuationReport() {
    ...
    useEffect(() => { loadValuation(); }, [asOfDate]);
    ...
    const barColors = [
        'bg-[#FFCC00]', 'bg-emerald-500', 'bg-blue-500', 'bg-purple-500',
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="flex items-center gap-3">
                ...
                />
                {asOfDate && (
                    <button onClick={() => setAsOfDate('')} className="text-xs text-zinc-500 hover:text-zinc-300">
                        Clear
                    </button>
                )}
            </div>
            ...
            <div className="neu-raised rounded-2xl p-6 border border-[#FFCC00]/10">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <p className="text-xs text-zinc-500 uppercase font-semibold tracking-wider">Total Inventory Value</p>
                        <p className="text-3xl font-bold text-gradient-gold">{formatAUD(data.totalValueCents)}</p>
                        <p className="text-xs text-zinc-500 mt-1">
                            {data.totalItems} items • {data.asOfDate ? `As of ${new Date(data.asOfDate).toLocaleDateString('en-AU')}` : 'Current'}
                        </p>
                    </div>
                </div>
            </div>
                        ...
                        <table className="w-full text-sm">
                            <thead>
                                <tr className="border-t border-b border-white/5">
                                    <th className="text-left px-5 py-3 text-xs font-bold text-zinc-500 uppercase">Category</th>
                                    <th className="text-right px-5 py-3 text-xs font-bold text-zinc-500 uppercase">Items</th>
                                    <th className="text-right px-5 py-3 text-xs font-bold text-zinc-500 uppercase">Total Value</th>
                                    <th className="text-right px-5 py-3 text-xs font-bold text-zinc-500 uppercase">% of Total</th>
                                </tr>
                            </thead>
                            <tbody>
                                {data.categories.map(cat => (
                                    <tr key={cat.category} className="border-b border-white/5">
                                        ...
                                        <td className="px-5 py-3 text-right text-zinc-200 font-medium">{formatAUD(cat.totalValueCents)}</td>
                                        <td className="px-5 py-3 text-right text-zinc-400">{cat.percentOfTotal.toFixed(1)}%</td>
                                    </tr>
                                ))}
                            </tbody>
                        </table>
            ...
            <div className="flex justify-end">
                <button className="px-4 py-2 rounded-xl neu-raised text-zinc-400 text-sm hover:text-zinc-200" disabled>
                    Export (Coming Soon)
                </button>
            </div>
        </div>
