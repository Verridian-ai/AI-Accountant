...
export function SplitModal({
  ...
  return (
    <div className="fixed inset-0 bg-black/80 backdrop-blur-xl z-100 flex items-center justify-center p-4 animate-in fade-in duration-300">
      ...
      <div className="neu-raised rounded-[2.5rem] w-full max-w-2xl overflow-hidden relative border border-border/50 shadow-2xl">
        {/* Header */}
            ...
            >
              <div className="flex justify-between items-center border-b border-border/50 pb-4">
                <div className="flex items-center gap-3">
                  <span className="w-6 h-6 neu-inset rounded-full flex items-center justify-center text-[10px] font-black text-cba-gold">
                    {idx + 1}
                  </span>
                  <span className="text-[10px] font-black uppercase text-muted tracking-[0.2em]">
                    Fragment Node
                  </span>
                </div>
                {splits.length > 2 && (
                ...
                <div className="space-y-2">
                  ...
                  <div className="relative">
                    <select
                      ...
                      className="w-full p-4 text-sm neu-inset rounded-2xl text-cba-gold font-bold bg-transparent border-none focus:ring-0 appearance-none"
                    >
                      {categories
                        .filter((c) => c !== 'All')
                        .map((c) => (
                    ...
                    <ChevronDown className="absolute right-4 top-1/2 -translate-y-1/2 h-4 w-4 text-cba-gold/50 pointer-events-none" />
                  </div>
                </div>
                <div className="space-y-2">
                  <label className="text-[9px] font-black uppercase text-zinc-600 tracking-widest ml-1">
        ...
        <div className="p-8 border-t border-border/50 bg-white/1 flex flex-col sm:flex-row justify-between items-center gap-6">
          <div className="flex items-center gap-4">
            <div className="flex flex-col">
              ...
              <span className="text-sm font-bold text-secondary">${splittingTx.amount / 100}</span>
            </div>
          ...
          <div className="flex gap-4 w-full sm:w-auto">
            <button
              onClick={() => setSplittingTx(null)}
              className="flex-1 sm:flex-none px-8 py-4 rounded-2xl text-muted font-bold text-xs uppercase tracking-widest neu-raised-sm btn-press"
            >
              Abort
            </button>
            <button
