...
export function MerchantMemoryManager({ isOpen, onClose }: MerchantMemoryManagerProps) {
  ...
  const handleSave = async (id: string) => {
    ...
    try {
      ...
      setMemory((prev) =>
        prev.map((m) =>
          m.id === id ? { ...m, category: editCategory, gstApplicable: editGst } : m,
        ),
      );
      setEditingId(null);
    } catch (err) {
  ...
  const handleDelete = async (id: string) => {
    if (
      !window.confirm(
        'Delete this learned pattern? Future transactions from this merchant will need to be categorized again.',
      )
    )
      return;
    try {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-md animate-in fade-in duration-300">
      ...
      <div className="neu-raised rounded-3xl w-full max-w-3xl max-h-[85vh] overflow-hidden flex flex-col relative animate-in zoom-in-95 duration-300 border border-border/50">
        {/* Header */}
        ...
        <div className="p-6 bg-white/[0.01]">
          <div className="relative group">
            <Search className="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-zinc-600 group-focus-within:text-cba-gold transition-colors" />
            <input
              type="text"
              value={search}
          ...
          {loading ? (
            <div className="space-y-4">
              {[...Array(5)].map((_, i) => (
                <div
                  key={i}
                  className="flex items-center gap-4 p-5 neu-raised-sm rounded-2xl border border-border/50"
                >
            ...
            <div className="text-center py-20">
              ...
              <p className="text-zinc-600 max-w-xs mx-auto text-sm font-medium">
                {search
                  ? `We couldn't find any patterns matching "${search}"`
                  : 'Categorize transactions in the main view to build your merchant intelligence.'}
              </p>
            </div>
          ) : (
              ...
              {filtered.map((item) => (
                ...
                >
                  <div className="w-10 h-10 neu-inset rounded-xl flex items-center justify-center shrink-0">
                    <Activity className="w-5 h-5 text-zinc-600 group-hover:text-cba-gold transition-colors" />
                  </div>

                  <div className="flex-1 min-w-0">
                    <p className="font-bold text-zinc-100 truncate text-base">
                  ...
                  {editingId === item.id ? (
                    <div className="flex items-center gap-3 animate-in fade-in slide-in-from-right-2 duration-300">
                      <div className="relative">
                        <select
                          ...
                          className="pl-3 pr-8 py-2 neu-inset rounded-xl text-sm font-bold text-cba-gold bg-transparent border-none focus:ring-0 appearance-none min-w-[160px]"
                        >
                          {CATEGORIES.map((cat) => (
                        ...
                        <Edit2 className="w-3 h-3 absolute right-3 top-1/2 -translate-y-1/2 text-muted pointer-events-none" />
                      </div>
                      ...
                      <button
                        ...
                        className={cn(
                          ...
                          editGst
                            ? 'cba-gold-bg text-base shadow-lg cba-gold-glow'
                      ...
                      <div className="flex gap-2">
                        ...
                        <button
                          ...
                          className="p-2 neu-raised-sm rounded-xl text-muted hover:text-primary btn-press"
                    ...
                    <div className="flex items-center gap-3">
                      <div className="flex flex-col items-end gap-1">
                        <span className="px-3 py-1.5 neu-inset rounded-full text-cba-gold text-[10px] font-black tracking-wider uppercase">
                          {item.category}
                        </span>
                        {item.gstApplicable && (
                          <span className="text-[9px] font-bold text-zinc-600 tracking-tighter italic">
                            + GST Applicable
                          </span>
                        )}
                      </div>
                      <div className="flex gap-2 ml-2">
                        <button
                          ...
                          className="p-2 neu-raised-sm rounded-xl text-muted hover:text-cba-gold btn-press group-hover:opacity-100 sm:opacity-0 transition-opacity"
                        ...
                        <button
                          ...
                          className="p-2 neu-raised-sm rounded-xl text-muted hover:text-red-400 btn-press group-hover:opacity-100 sm:opacity-0 transition-opacity"
        ...
        {/* Footer */}
        <div className="px-8 py-5 border-t border-border/50 bg-white/[0.01] flex items-center justify-between">
            ...
            <div className="flex -space-x-2">
              {[...Array(3)].map((_, i) => {
                ...
                return (
                  <div
                    key={i}
                    className={`w-2 h-2 rounded-full cba-gold-bg opacity-30 animate-pulse ${delayClass}`}
                  />
                );
              })}
            </div>
          ...
          <p className="text-[11px] font-bold text-muted italic">
            <span className="text-secondary font-black">{memory.length}</span> patterns indexed
          </p>
        </div>
      </div>
    </div>
