...
function calculateLoan(
  ...
  if (repaymentType === 'io') {
    ...
    if (monthlyRate === 0) {
      ...
      monthlyRepayment = effectiveAmount * (monthlyRate * Math.pow(1 + monthlyRate, totalMonths)) / (Math.pow(1 + monthlyRate, totalMonths) - 1);
    }
  }
...
export function LoanComparison() {
  ...
  const summaryCards = [
    { label: 'Monthly Repayment', value: formatCurrency(result.monthlyRepayment), icon: DollarSign, color: 'text-[#FFCC00]' },
    { label: 'Total Interest', value: formatCurrency(result.totalInterest), icon: TrendingDown, color: 'text-red-400' },
    { label: 'Payoff Date', value: result.payoffDate, icon: Calendar, color: 'text-emerald-400' },
    { label: 'Interest Saved', value: formatCurrency(result.interestSaved), icon: Calculator, color: 'text-cyan-400' },
  ];
    ...
    <div className="space-y-6">
      ...
      <div className="neu-raised rounded-2xl p-5 space-y-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          ...
          <div>
            <label className="text-xs text-zinc-500 font-semibold block mb-1">Interest Rate (%)</label>
            <input
              type="number"
              step="0.01"
              value={rate}
          ...
          <div>
            <label className="text-xs text-zinc-500 font-semibold block mb-1">Extra Repayment/mo</label>
            <input
              type="number"
      ...
      <div className="neu-raised rounded-2xl p-5 space-y-3">
        ...
        <div className="space-y-2">
          <div className="flex justify-between text-sm">
            ...
            <span className="text-zinc-200 font-semibold">{formatCurrency(Math.max(0, amount - offsetBalance))}</span>
          </div>
          <div className="flex justify-between text-sm">
            <span className="text-zinc-400">Total Interest</span>
            <span className="text-red-400 font-semibold">{formatCurrency(result.totalInterest)}</span>
          </div>
