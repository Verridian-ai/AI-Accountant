...
export function DataHolderDirectory() {
  ...
  const statusColor = (status: string) => {
    switch (status.toLowerCase()) {
      case 'active': case 'success': case 'completed': return 'text-emerald-400';
      case 'error': case 'failed': return 'text-red-400';
      case 'pending': case 'crawling': return 'text-amber-400';
  ...
  const statusIcon = (status: string) => {
    switch (status.toLowerCase()) {
      case 'active': case 'success': case 'completed': return Check;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <p className="text-sm text-zinc-500">{holders.length} data holder{holders.length !== 1 ? 's' : ''} registered</p>
        <button
          onClick={handleCrawl}
          disabled={crawling}
          className={cn(
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-white/5">
                  <th className="text-left py-2 px-3 text-xs text-zinc-500 font-semibold">Provider</th>
                  <th className="text-left py-2 px-3 text-xs text-zinc-500 font-semibold">Status</th>
                  <th className="text-right py-2 px-3 text-xs text-zinc-500 font-semibold">Products</th>
                  <th className="text-right py-2 px-3 text-xs text-zinc-500 font-semibold">Duration</th>
              ...
              <tbody>
                {crawlLogs.slice(0, 20).map(log => (
                  <tr key={log.id} className="border-b border-white/5 last:border-0">
                    ...
                    <td className={cn("py-2 px-3 font-semibold", statusColor(log.status))}>{log.status}</td>
                    <td className="py-2 px-3 text-right text-zinc-400">{log.productsFound}</td>
                    <td className="py-2 px-3 text-right text-zinc-500">{(log.duration / 1000).toFixed(1)}s</td>
