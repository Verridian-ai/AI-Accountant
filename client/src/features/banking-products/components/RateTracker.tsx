...
import { fetchMarketRates } from '@/api';
...
export function RateTracker() {
  ...
  useEffect(() => {
    const load = async () => {
      setLoading(true);
      try {
        const result = await fetchMarketRates(category);
        setData(result);
      } catch (e) {
  ...
  const summaryCards = [
    { label: 'Lowest Variable', value: formatRate(data?.lowestVariable ?? null), icon: TrendingDown, color: 'text-emerald-400' },
    { label: 'Lowest 2yr Fixed', value: formatRate(data?.lowestFixed2yr ?? null), icon: TrendingDown, color: 'text-cyan-400' },
    { label: 'Average Variable', value: formatRate(data?.averageVariable ?? null), icon: TrendingUp, color: 'text-amber-400' },
    { label: 'Products Tracked', value: String(data?.productCount ?? 0), icon: Hash, color: 'text-[#FFCC00]' },
  ];
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {summaryCards.map(card => (
          <div key={card.label} className={cn("neu-raised rounded-2xl p-5", loading && "animate-pulse")}>
            <div className="flex items-center gap-2 mb-2">
              <div className="neu-inset p-1.5 rounded-lg">
                <card.icon className={cn("h-4 w-4", card.color)} />
              </div>
              <span className="text-xs text-zinc-500 font-semibold uppercase tracking-wider">{card.label}</span>
            </div>
            ...
            {(data?.rateDistribution ?? []).map((bucket, i) => {
              ...
              return (
                <div key={i} className="flex-1 flex flex-col items-center gap-1">
                  ...
                  />
                  <span className="text-[10px] text-zinc-600 truncate w-full text-center">{bucket.bucket}</span>
                </div>
              );
            })}
            {(!data?.rateDistribution || data.rateDistribution.length === 0) && (
              <div className="flex-1 text-center text-zinc-600 text-sm py-16">No distribution data</div>
            )}
          </div>
        )}
      </div>
        ...
        {loading ? (
          ...
          <div className="space-y-2">
            {(data?.providerComparison ?? []).slice(0, 15).map((prov, i) => {
              const maxRate = Math.max(...(data?.providerComparison ?? []).map(p => p.avgRate), 0.001);
              const width = (prov.avgRate / maxRate) * 100;
              return (
                <div key={i} className="flex items-center gap-3">
                  <span className="text-xs text-zinc-400 w-32 truncate shrink-0">{prov.provider}</span>
                  <div className="flex-1 h-6 bg-[#1a1d23] rounded-lg overflow-hidden">
                    <div
                      ...
                      style={{ width: `${width}%` }}
                    >
                      <span className="text-[10px] font-bold text-[#0a0a0f]">{(prov.avgRate * 100).toFixed(2)}%</span>
                    </div>
                  </div>
                  <span className="text-[10px] text-zinc-600 w-16 text-right shrink-0">{prov.productCount} products</span>
                </div>
              );
            })}
