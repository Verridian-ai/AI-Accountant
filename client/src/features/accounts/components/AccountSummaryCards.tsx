...
import { Wallet, CreditCard, PiggyBank, Building2, TrendingUp, TrendingDown, Clock, Receipt } from 'lucide-react';
  ...
  if (loading) {
    return (
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {[...Array(3)].map((_, i) => (
          <div key={i} className="h-44 neu-raised rounded-2xl animate-pulse border border-white/5" />
        ))}
      </div>
    );
  }
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-2">
        ...
        <h3 className="text-[10px] font-black text-zinc-500 uppercase tracking-[0.25em]">Account Summary</h3>
      </div>

      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {summaries.map(({ account, transactionCount, totalIncome, totalExpenses, lastActivity }) => {
          ...
          return (
            <div key={account.id} className="neu-raised rounded-2xl p-5 border border-white/5 group hover:border-[#FFCC00]/10 transition-all">
              {/* Header */}
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-1.5 mt-0.5">
                      ...
                      {ownershipTag && (
                        <Badge variant={ownershipTag === 'personal' ? 'outline' : 'default'} className="text-[7px]">
                          {ownershipTag}
                        </Badge>
                      )}
                    </div>
              ...
              <div className="grid grid-cols-2 gap-2">
                <div className="neu-inset rounded-lg p-2.5">
                  <div className="flex items-center gap-1.5 mb-1">
                    ...
                    <span className="text-[7px] font-black text-zinc-600 uppercase tracking-widest">Income</span>
                  </div>
                  <p className="text-xs font-black text-emerald-400">{formatCurrency(totalIncome)}</p>
                </div>
                <div className="neu-inset rounded-lg p-2.5">
                  <div className="flex items-center gap-1.5 mb-1">
                    <TrendingDown className="w-3 h-3 text-red-400" />
                    <span className="text-[7px] font-black text-zinc-600 uppercase tracking-widest">Expenses</span>
                  </div>
                  <p className="text-xs font-black text-red-400">{formatCurrency(totalExpenses)}</p>
                </div>
                <div className="neu-inset rounded-lg p-2.5">
                  <div className="flex items-center gap-1.5 mb-1">
                    <Receipt className="w-3 h-3 text-[#FFCC00]" />
                    <span className="text-[7px] font-black text-zinc-600 uppercase tracking-widest">Transactions</span>
                  </div>
                ...
                <div className="neu-inset rounded-lg p-2.5">
                  <div className="flex items-center gap-1.5 mb-1">
                    ...
                    <span className="text-[7px] font-black text-zinc-600 uppercase tracking-widest">Last Active</span>
                  </div>
                  <p className="text-[10px] font-bold text-zinc-400">{formatDate(lastActivity)}</p>
                </div>
              </div>
            </div>
          );
