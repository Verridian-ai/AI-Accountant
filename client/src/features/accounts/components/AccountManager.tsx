    ...
    return (
      <div className="neu-raised rounded-3xl p-8 border border-white/5">
        <div className="flex items-center gap-3 mb-6">
          ...
          <span className="text-[10px] font-black text-zinc-500 uppercase tracking-[0.2em]">Loading accounts...</span>
        </div>
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between mb-2">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-[10px] font-black text-zinc-500 uppercase tracking-[0.25em]">Account Manager</h2>
        </div>
        <Badge variant="secondary" className="text-[9px]">{accounts.length} accounts</Badge>
      </div>
            ...
            >
                  ...
                  <div>
                    {isEditing ? (
                      <input
                        ...
                        onChange={(e) => setEditValues(prev => ({ ...prev, accountName: e.target.value }))}
              ...
              {isEditing && (
                <div className="space-y-3 mb-4">
                  <div>
                    <label className="text-[8px] font-black text-zinc-600 uppercase tracking-[0.2em] block mb-1.5">Type</label>
                    <select
                      value={editValues.accountType || account.accountType}
                      onChange={(e) => setEditValues(prev => ({ ...prev, accountType: e.target.value }))}
                    ...
                    >
                      {ACCOUNT_TYPES.map(t => (
                        <option key={t} value={t} className="bg-[#0a0a0f]">{t.replace('_', ' ')}</option>
                      ))}
                    </select>
                  </div>
                  <div>
                  ...
                  <div>
                    <label className="text-[8px] font-black text-zinc-600 uppercase tracking-[0.2em] block mb-1.5">Business Account</label>
                    <button
                      type="button"
              ...
              <div className="flex items-center gap-3 mb-3">
                <div className="flex items-center gap-1 text-[8px] font-bold text-zinc-600 uppercase tracking-wider">
                  ...
                  <span>{(account as Account & { transactionCount?: number }).transactionCount ?? '\u2014'} txns</span>
                </div>
