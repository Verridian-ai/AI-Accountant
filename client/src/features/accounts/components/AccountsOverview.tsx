...
import { Wallet, CreditCard, PiggyBank, Building2, AlertTriangle, ChevronRight, Loader2, Activity, ShieldCheck, Zap } from 'lucide-react';
            ...
            <div className="space-y-6">
                ...
                <div className="neu-raised rounded-3xl overflow-hidden flex flex-col border border-white/5">
                    ...
                    <div className="p-4 space-y-3 bg-white/[0.005]">
                         {[...Array(3)].map((_, i) => (
                             <div key={i} className="flex items-center gap-4 p-4 neu-raised-sm rounded-2xl border border-white/5">
    ...
    if (accounts.length === 0) {
        return (
            <div className="neu-raised rounded-3xl p-10 text-center border border-white/5">
                ...
                <h3 className="font-black text-zinc-200 uppercase tracking-widest text-sm">No Active Vaults</h3>
            ...
            {alerts.length > 0 && (
                <div className="neu-raised rounded-3xl p-6 border border-amber-500/10 relative overflow-hidden group">
                    ...
                    <div className="flex items-center gap-3 mb-5 relative">
                        ...
                        <div>
                            <h3 className="text-[11px] font-black text-amber-400 uppercase tracking-[0.2em]">Discrepancy Alerts</h3>
                            <p className="text-[9px] font-bold text-zinc-600 uppercase mt-0.5">{alerts.length} conflicts detected</p>
                        </div>
                    </div>
                    <div className="space-y-3 relative">
                        {alerts.slice(0, 3).map((alert) => (
                            <div key={alert.id} className="flex items-center justify-between neu-inset rounded-2xl p-4 border border-white/5 hover:border-amber-500/20 transition-all">
                                <div className="flex-1 min-w-0 pr-4">
                                    <p className="text-xs font-black text-zinc-300 uppercase tracking-tight truncate">{alert.description}</p>
            ...
            <div className="neu-raised rounded-3xl overflow-hidden flex flex-col border border-white/5">
                <div className="px-6 py-5 border-b border-white/5 bg-white/[0.01] flex items-center justify-between">
                    <div className="flex items-center gap-3">
                        ...
                        <h3 className="text-xs font-black text-zinc-100 uppercase tracking-[0.2em]">Vault Inventory</h3>
                    </div>
                        ...
                        return (
                            ...
                            >
                                <div className={cn(
                                    ...
                                    account.accountType === 'credit_card' ? "text-purple-400 group-hover:text-purple-300" : "text-[#FFCC00] group-hover:text-[#FFE066]"
                                )}>
                                ...
                                <div className="flex-1 min-w-0">
                                    ...
                                    <div className="flex items-center gap-2 mt-1">
                                        <span className="text-[9px] font-bold text-zinc-600 uppercase tracking-widest">{account.accountNumber}</span>
                                        <span className="w-1 h-1 rounded-full bg-zinc-800" />
                                        <span className="text-[9px] font-bold text-zinc-500 uppercase tracking-tight italic">{account.bankName || 'Verified Source'}</span>
                                    </div>
                                </div>
                ...
                <div className="px-6 py-3 border-t border-white/5 bg-black/20">
                    <p className="text-[8px] font-black text-zinc-700 uppercase tracking-[0.3em] text-center">Biometric Encryption Active</p>
                </div>
            </div>
        </div>
    );
