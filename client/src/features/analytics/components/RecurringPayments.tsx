...
export function RecurringPayments() {
  ...
  const grouped = useMemo(() => {
    ...
    const sorted = [...payments].sort((a, b) => {
      ...
      if (sortBy === 'nextDue') return new Date(a.nextExpected).getTime() - new Date(b.nextExpected).getTime();
      return 0;
    });
  ...
  const totalMonthly = useMemo(() => {
    return payments.reduce((sum, p) => {
      switch (p.frequency) {
        case 'weekly': return sum + p.typicalAmount * 4.33;
        case 'fortnightly': return sum + p.typicalAmount * 2.17;
        case 'monthly': return sum + p.typicalAmount;
        case 'quarterly': return sum + p.typicalAmount / 3;
        case 'annual': return sum + p.typicalAmount / 12;
        default: return sum;
      }
    }, 0);
  }, [payments]);
  ...
  if (payments.length === 0) {
    return (
      <div className="neu-raised rounded-3xl p-10 text-center border border-white/5">
        ...
        <h3 className="font-black text-zinc-200 uppercase tracking-widest text-sm">No Recurring Payments</h3>
      ...
      <div className="neu-raised rounded-2xl p-4 border border-white/5">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div className="flex items-center gap-2">
            ...
            <span className="text-[10px] font-black text-zinc-500 uppercase tracking-[0.2em]">Recurring Payments</span>
          </div>
          <div className="flex gap-3">
            <div className="text-right">
              <p className="text-[8px] font-black text-zinc-600 uppercase tracking-widest">Monthly</p>
              <p className="text-sm font-black text-red-400">{formatCurrency(totalMonthly)}</p>
            </div>
            <div className="w-px bg-white/10" />
            <div className="text-right">
              <p className="text-[8px] font-black text-zinc-600 uppercase tracking-widest">Annual</p>
          ...
          <div key={freq} className="neu-raised rounded-2xl border border-white/5 overflow-hidden">
            ...
            >
              <div className="flex items-center gap-2">
                ...
                <Badge variant="secondary" className="text-[8px]">{items.length}</Badge>
              </div>
              {isCollapsed ? <ChevronDown className="w-4 h-4 text-zinc-600" /> : <ChevronUp className="w-4 h-4 text-zinc-600" />}
            </button>

            {!isCollapsed && (
              <div className="divide-y divide-white/5">
                {items.map(payment => (
                  <div key={payment.id} className="px-5 py-3 flex items-center gap-4 hover:bg-white/[0.02] transition-colors">
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2">
                        <p className="text-xs font-bold text-zinc-200 truncate">{payment.merchantPattern}</p>
                        {payment.isMissed && (
                          <Badge variant="destructive" className="text-[7px] px-1.5 py-0">
                            <AlertCircle className="w-2.5 h-2.5 mr-0.5" />MISSED
                          </Badge>
                        )}
                        {payment.amountChanged && (
                          <Badge variant="warning" className="text-[7px] px-1.5 py-0">
                            <TrendingUp className="w-2.5 h-2.5 mr-0.5" />CHANGED
                          </Badge>
                        )}
                      </div>
                      <div className="flex items-center gap-3 mt-1">
                        {payment.category && (
                          <span className="text-[8px] font-bold text-zinc-600 uppercase tracking-tight">{payment.category}</span>
                        )}
                        <span className="text-[8px] text-zinc-600">Next: {formatDate(payment.nextExpected)}</span>
                      </div>
                    </div>
                    <div className="text-right shrink-0">
                      <p className="text-sm font-black text-red-400 tabular-nums">{formatCurrency(payment.typicalAmount)}</p>
