...
export function BillAlerts() {
    ...
    const statusIcon = (status: RecurringBill['status']) => {
        switch (status) {
            ...
            case 'amount_changed': return <TrendingUp className="w-4 h-4 text-amber-400" />;
            case 'current': return <CheckCircle className="w-4 h-4 text-emerald-400" />;
        }
    };

    const statusColor = (status: RecurringBill['status']) => {
        switch (status) {
            case 'overdue': return 'border-red-500/20 bg-red-500/5';
            case 'amount_changed': return 'border-amber-500/20 bg-amber-500/5';
            case 'current': return 'border-white/5';
        }
    };

    const renderBill = (bill: RecurringBill) => (
        <div key={bill.merchant} className={`flex items-center justify-between p-4 rounded-lg border ${statusColor(bill.status)}`}>
            ...
            <div className="text-right">
                ...
                {bill.status === 'amount_changed' && bill.amountChangePercent !== undefined && (
                    <p className={`text-xs ${bill.amountChangePercent > 0 ? 'text-red-400' : 'text-emerald-400'}`}>
                        {bill.amountChangePercent > 0 ? '+' : ''}{bill.amountChangePercent.toFixed(1)}% change
                    </p>
                )}
            ...
            {overdue.length > 0 && (
                <Card className="neu-raised border-red-500/20">
                    <CardHeader>
                        ...
                        <CardDescription>Bills that appear to be past their expected payment date</CardDescription>
                    </CardHeader>
            ...
            {amountChanged.length > 0 && (
                <Card className="neu-raised border-amber-500/20">
                    <CardHeader>
                        ...
                        <CardDescription>Bills where the amount has changed compared to the average</CardDescription>
                    </CardHeader>
                    <CardContent className="space-y-2">
                        {amountChanged.map(renderBill)}
                    </CardContent>
                </Card>
            )}
