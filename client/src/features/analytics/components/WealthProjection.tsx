                ...
                <>
                    ...
                    <Card className="neu-raised border-white/5">
                        <CardHeader>
                            ...
                            <CardDescription>Nominal and inflation-adjusted values across risk profiles</CardDescription>
                        </CardHeader>
                        <CardContent>
                            <div className="overflow-x-auto">
                                <table className="w-full text-sm">
                                    ...
                                    <tbody>
                                        {result.profiles[0]?.projections.map((_, idx) => (
                                            <tr key={idx} className="border-b border-white/5 last:border-0">
                                                <td className="py-2 pr-4 text-zinc-400">{result.profiles[0]?.projections[idx]?.years}</td>
                                                {result.profiles.map(p => (
                                                    <td key={p.name} className="text-right py-2 px-1" colSpan={2}>
                                                        <span className="mr-3 font-medium">{formatCurrency(p.projections[idx]?.nominalValue ?? 0)}</span>
                                                        <span className="text-zinc-500">{formatCurrency(p.projections[idx]?.realValue ?? 0)}</span>
                                                    </td>
                                                ))}
                                            </tr>
                                        ))}
