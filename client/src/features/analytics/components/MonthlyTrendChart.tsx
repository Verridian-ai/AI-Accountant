  ...
  const monthlyData = useMemo(() => {
    ...
    return Object.entries(groups)
      ...
      .map(([month, amount]) => ({
        month,
        amount,
        label: new Date(month + '-01').toLocaleDateString('en-AU', {
          month: 'short',
          year: '2-digit',
        }),
      }));
        ...
        <div className="flex items-end justify-between gap-3 h-40 px-2">
          {skeletonHeights.map((height, i) => (
            <div key={i} className="flex-1 flex flex-col items-center gap-3">
              <Skeleton className="w-full rounded-xl" style={{ height: `${height}%` }} />
    ...
    <div className="neu-raised rounded-3xl p-6 border border-border/50 flex flex-col group">
      <div className="flex items-center justify-between mb-10">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h3 className="text-xs font-black text-zinc-100 uppercase tracking-[0.2em] leading-none">
              {title}
            </h3>
      ...
      <div className="flex items-end justify-between gap-3 h-40 px-2">
        {monthlyData.map((d) => {
          ...
          return (
            <div
              key={d.month}
              className="flex-1 flex flex-col items-center gap-3 group/bar relative"
            >
              {/* Augmented Tooltip */}
