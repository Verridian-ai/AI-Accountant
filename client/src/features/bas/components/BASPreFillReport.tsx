...
export function BASPreFillReport() {
    ...
    const LabelRow = ({ label, description, amount, count }: { label: string; description: string; amount: number; count?: number }) => (
        <div className="flex items-center justify-between py-2.5 px-3 rounded-lg hover:bg-white/[0.02] cursor-pointer group transition-colors">
            <div className="flex items-center gap-3">
                ...
                {count !== undefined && (
                    <span className="text-[10px] text-zinc-600">({count} txns)</span>
                )}
            </div>
                ...
                <>
                    ...
                    {basData.warnings.length > 0 && (
                        <div className="space-y-1">
                            {basData.warnings.map((warning, i) => (
                                <div key={i} className="flex items-center gap-2 text-xs text-amber-400 bg-amber-500/5 px-3 py-2 rounded-lg border border-amber-500/10">
                    ...
                    <Card className="neu-raised rounded-xl border-l-2 border-l-[#FFCC00]/30">
                        ...
                        <CardContent className="space-y-0.5">
                            <LabelRow label="G1" description="Total Sales" amount={basData.labels.G1} count={basData.transactionCounts?.G1} />
                            {basMethod === 'full' && (
                                <>
                                    {basData.labels.G2 !== undefined && <LabelRow label="G2" description="Export Sales" amount={basData.labels.G2} />}
                                    {basData.labels.G3 !== undefined && <LabelRow label="G3" description="GST-Free Sales" amount={basData.labels.G3} />}
                                    {basData.labels.G4 !== undefined && <LabelRow label="G4" description="Input Taxed Sales" amount={basData.labels.G4} />}
                                </>
                            )}
                    ...
                    <Card className="neu-raised rounded-xl border-l-2 border-l-blue-500/30">
                        ...
                        <CardContent className="space-y-0.5">
                            {basMethod === 'full' && (
                                <>
                                    {basData.labels.G10 !== undefined && <LabelRow label="G10" description="Capital Purchases" amount={basData.labels.G10} />}
                                    {basData.labels.G11 !== undefined && <LabelRow label="G11" description="Non-Capital Purchases" amount={basData.labels.G11} />}
                                </>
                            )}
                            <div className={basMethod === 'full' ? 'border-t border-white/5 mt-2 pt-1' : ''}>
                                <LabelRow label="1B" description="GST on Purchases" amount={basData.labels['1B']} />
                            </div>
                        </CardContent>
                    </Card>
                    ...
                    {(basData.labels.W1 !== undefined || basData.labels.W2 !== undefined) && (
                        <Card className="neu-raised rounded-xl border-l-2 border-l-purple-500/30">
                            ...
                            <CardContent className="space-y-0.5">
                                {basData.labels.W1 !== undefined && <LabelRow label="W1" description="Gross Wages" amount={basData.labels.W1} />}
                                {basData.labels.W2 !== undefined && <LabelRow label="W2" description="Amounts Withheld" amount={basData.labels.W2} />}
                                {basData.labels['5A'] !== undefined && <LabelRow label="5A" description="PAYG Instalment" amount={basData.labels['5A']} />}
                            </CardContent>
                        </Card>
                    )}
                        ...
                        <CardContent>
                            <div className="space-y-2 text-sm">
                                ...
                                <div className="border-t border-white/10 pt-2 flex justify-between text-base font-bold">
                                    ...
                                    <span className={basData.totalPayable > 0 ? 'text-red-400' : 'text-emerald-400'}>
