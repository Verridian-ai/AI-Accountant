...
export function AssetDisposalForm({ assets, onSuccess }: AssetDisposalFormProps) {
    ...
    const selectedAsset = useMemo(() => activeAssets.find(a => a.id === selectedAssetId), [activeAssets, selectedAssetId]);
                    ...
                    <>
                        <div className="grid gap-4 md:grid-cols-2">
                            ...
                            <div className="space-y-2">
                                ...
                                <Input placeholder="Optional" value={buyerDetails} onChange={e => setBuyerDetails(e.target.value)} />
                            </div>
                            <div className="space-y-2">
                                <Label>Invoice Reference</Label>
                                <Input placeholder="Optional" value={invoiceRef} onChange={e => setInvoiceRef(e.target.value)} />
                            </div>
                        </div>
                        <div className="space-y-2">
                            <Label>Notes</Label>
                            ...
                            <div className="neu-inset rounded-xl p-4 space-y-3">
                                ...
                                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                                    <div>
                                        ...
                                        <p className="font-mono text-zinc-200">{formatCurrency(selectedAsset.purchasePrice)}</p>
                                    </div>
                                    <div>
                                    ...
                                    <div>
                                        <p className="text-zinc-500 text-xs">{gainLoss >= 0 ? 'Profit' : 'Loss'} on Disposal</p>
                                        <p className={`font-mono font-bold ${gainLoss >= 0 ? 'text-emerald-400' : 'text-red-400'}`}>
                                ...
                                {isCgtApplicable && (
                                    <div className="flex items-center gap-2 mt-2">
                                        ...
                                        <span className="text-xs text-amber-400">CGT may apply — original cost exceeds $10,000</span>
                                    </div>
                                )}
                            </div>
                        )}
