...
export function DepreciationScheduleView({ schedule, loading, financialYear, entityId, onRefresh }: DepreciationScheduleViewProps) {
                    ...
                    {!schedule || schedule.assets.length === 0 ? (
                        ...
                        <div className="overflow-x-auto">
                            <table className="w-full text-sm">
                                ...
                                <tbody>
                                    {schedule.assets.map((item) => (
                                        <tr key={item.assetId} className="border-b border-white/5 hover:bg-white/[0.02] transition-colors">
                                            ...
                                            <td className="py-3 px-2 text-right font-mono text-zinc-300">{formatCurrency(item.openingValue)}</td>
                                            <td className="py-3 px-2 text-right font-mono text-emerald-400">
                                                {item.additions > 0 ? formatCurrency(item.additions) : '—'}
                                            </td>
                                            <td className="py-3 px-2 text-right font-mono text-red-400">{formatCurrency(item.depreciation)}</td>
                                            <td className="py-3 px-2 text-right font-mono text-amber-400">
                                                {item.disposals > 0 ? formatCurrency(item.disposals) : '—'}
                                            </td>
                                            <td className="py-3 px-2 text-right font-mono text-[#FFCC00] font-bold">{formatCurrency(item.closingValue)}</td>
                                        </tr>
                                    ))}
                                </tbody>
                                <tfoot>
                                    <tr className="border-t-2 border-[#FFCC00]/30 font-bold">
                                        <td colSpan={3} className="py-3 px-2 text-zinc-300">Totals</td>
                                        <td className="py-3 px-2 text-right font-mono text-zinc-100">{formatCurrency(schedule.totals.openingValue)}</td>
                                        <td className="py-3 px-2 text-right font-mono text-emerald-400">{formatCurrency(schedule.totals.totalAdditions)}</td>
                                        <td className="py-3 px-2 text-right font-mono text-red-400">{formatCurrency(schedule.totals.totalDepreciation)}</td>
                                        <td className="py-3 px-2 text-right font-mono text-amber-400">{formatCurrency(schedule.totals.totalDisposals)}</td>
                                        <td className="py-3 px-2 text-right font-mono text-[#FFCC00]">{formatCurrency(schedule.totals.closingValue)}</td>
                                    </tr>
                                </tfoot>
                            </table>
                        </div>
                    ...
                    <CardContent className="pt-6">
                        ...
                        <div className="grid grid-cols-2 gap-4 text-sm">
                            ...
                            <div>
                                ...
                                <p className="text-xl font-bold text-[#FFCC00]">{formatCurrency(batchResult.totalDepreciation)}</p>
                            </div>
                        </div>
