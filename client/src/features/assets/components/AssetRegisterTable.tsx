...
type SortKey = 'assetNumber' | 'assetName' | 'category' | 'purchaseDate' | 'purchasePrice' | 'currentWrittenDownValue' | 'status';
                        ...
                        <table className="w-full text-sm">
                            <thead>
                                <tr className="border-b border-white/10 text-zinc-500 text-xs uppercase tracking-wider">
                                    <th className="text-left py-3 px-2 cursor-pointer hover:text-zinc-300" onClick={() => handleSort('assetNumber')}>
                                        Asset # <SortIcon col="assetNumber" />
                                    </th>
                                    <th className="text-left py-3 px-2 cursor-pointer hover:text-zinc-300" onClick={() => handleSort('assetName')}>
                                        Name <SortIcon col="assetName" />
                                    </th>
                                    <th className="text-left py-3 px-2 cursor-pointer hover:text-zinc-300" onClick={() => handleSort('category')}>
                                        Category <SortIcon col="category" />
                                    </th>
                                    <th className="text-left py-3 px-2 cursor-pointer hover:text-zinc-300" onClick={() => handleSort('purchaseDate')}>
                                        Purchase <SortIcon col="purchaseDate" />
                                    </th>
                                    <th className="text-right py-3 px-2 cursor-pointer hover:text-zinc-300" onClick={() => handleSort('purchasePrice')}>
                                        Cost <SortIcon col="purchasePrice" />
                                    </th>
                                    <th className="text-right py-3 px-2 cursor-pointer hover:text-zinc-300" onClick={() => handleSort('currentWrittenDownValue')}>
                                        WDV <SortIcon col="currentWrittenDownValue" />
                                    </th>
                                    <th className="text-left py-3 px-2 cursor-pointer hover:text-zinc-300" onClick={() => handleSort('status')}>
                                    ...
                                    <>
                                        ...
                                        >
                                            ...
                                            <td className="py-3 px-2">
                                                <Badge variant="outline" className={CATEGORY_COLORS[asset.category] ?? CATEGORY_COLORS['Other']}>
                                                    {asset.category}
                                                </Badge>
                                            </td>
                                            <td className="py-3 px-2 text-zinc-400">{asset.purchaseDate}</td>
                                            <td className="py-3 px-2 text-right font-mono text-zinc-200">{formatCurrency(asset.purchasePrice)}</td>
                                            <td className="py-3 px-2 text-right font-mono text-[#FFCC00]">{formatCurrency(asset.currentWrittenDownValue)}</td>
                                            <td className="py-3 px-2">
                                            ...
                                            <td className="py-3 px-2 text-right">
                                                <div className="flex justify-end gap-1" onClick={e => e.stopPropagation()}>
                                                    {onEdit && (
                                                        <Button variant="ghost" size="sm" className="h-7 w-7 p-0 text-zinc-500 hover:text-[#FFCC00]" onClick={() => onEdit(asset)}>
                                                            <Edit className="w-3.5 h-3.5" />
                                                        </Button>
                                                    )}
                                                    {onDispose && asset.status === 'active' && (
                                                        <Button variant="ghost" size="sm" className="h-7 w-7 p-0 text-zinc-500 hover:text-red-400" onClick={() => onDispose(asset)}>
                                                ...
                                                <td colSpan={8} className="p-4">
                                                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                                                        <div>
                                                            ...
                                                            <p className="text-zinc-200 font-medium">{asset.depreciationMethod}</p>
                                                        </div>
                                                        <div>
                                                            <p className="text-zinc-500 text-xs">Effective Life</p>
                                                            <p className="text-zinc-200 font-medium">{asset.effectiveLifeYears} years</p>
                                                        </div>
                                                        <div>
                                                            <p className="text-zinc-500 text-xs">Residual Value</p>
                                                            <p className="text-zinc-200 font-medium">{formatCurrency(asset.residualValue)}</p>
                                                        </div>
                                                        {asset.location && (
                                                            <div>
