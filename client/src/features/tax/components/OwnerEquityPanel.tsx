...
export function OwnerEquityPanel({ year }: { year: string }) {
    ...
    const [detected, setDetected] = useState<{ contributions: DetectedEquityEvent[]; drawings: DetectedEquityEvent[] } | null>(null);
    const [error, setError] = useState<string | null>(null);

    useEffect(() => {
        setLoading(true);
        taxApi.fetchEquitySummary(year)
            .then(setSummary)
        ...
        <div className="space-y-6">
            ...
            <div className="grid gap-4 md:grid-cols-3">
                ...
                <Card className="neu-raised border-white/5">
                    ...
                    <CardContent>
                        <div className={`text-2xl font-bold ${(summary?.netEquityChange ?? 0) >= 0 ? 'text-emerald-400' : 'text-red-400'}`}>
            ...
            <div className="flex justify-end">
                <Button onClick={handleScan} disabled={scanning} className="bg-[#FFCC00] text-[#0a0a0f] hover:bg-[#e6b800]">
            ...
            {detected && (detected.contributions.length > 0 || detected.drawings.length > 0) && (
                <Card className="neu-raised border-[#FFCC00]/10">
                    <CardHeader>
                        ...
                        <CardDescription>Review and confirm detected contributions and drawings</CardDescription>
                    </CardHeader>
                    <CardContent>
                        <div className="space-y-3">
                            {detected.contributions.map(e => (
                                <div key={e.transactionId} className="flex items-center justify-between p-3 rounded-lg border border-emerald-500/10 bg-emerald-500/5">
                                    <div className="flex items-center gap-3">
                                        <ArrowUpRight className="w-5 h-5 text-emerald-400 shrink-0" />
                                        <div>
                                            <p className="font-medium text-sm">{e.description}</p>
                                            <p className="text-xs text-zinc-500">{e.date} • {e.sourceAccount}</p>
                                        </div>
                                    </div>
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div className="flex gap-1">
                                            <Button size="sm" variant="ghost" onClick={() => handleConfirm(e.transactionId, true)} className="text-emerald-400 hover:text-emerald-300 p-1 h-auto">
                                                <CheckCircle2 className="w-5 h-5" />
                                            </Button>
                                            <Button size="sm" variant="ghost" onClick={() => handleConfirm(e.transactionId, false)} className="text-red-400 hover:text-red-300 p-1 h-auto">
                            ...
                            {detected.drawings.map(e => (
                                <div key={e.transactionId} className="flex items-center justify-between p-3 rounded-lg border border-red-500/10 bg-red-500/5">
                                    <div className="flex items-center gap-3">
                    ...
                    <CardContent>
                        <div className="space-y-2">
                            ...
                            {summary.monthlyBreakdown.map(m => (
                                <div key={m.month} className="grid grid-cols-4 text-sm py-1.5 border-b border-white/5 last:border-0">
                                    <span className="text-zinc-400">{m.month}</span>
                                    <span className="text-right text-emerald-400">{formatCurrency(m.contributions)}</span>
                                    <span className="text-right text-red-400">{formatCurrency(m.drawings)}</span>
                                    <span className={`text-right font-medium ${m.net >= 0 ? 'text-emerald-400' : 'text-red-400'}`}>
