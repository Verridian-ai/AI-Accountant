...
export function TaxDashboard() {
    ...
    const [entityType, setEntityType] = useState<'individual' | 'company' | 'trust' | 'super_fund'>('individual');
    ...
    const calculateTax = async () => {
        ...
        try {
            ...
            const result = await taxApi.calculateTax(selectedYear, incomeInCents, entityType, hasPrivateHealth);
            setTaxResult(result);
        } catch (err) {
                ...
                <TabsContent value="calculator" className="space-y-4">
                    <Card>
                        <CardHeader>
                            ...
                            <CardDescription>
                                Calculate your estimated tax for {selectedYear}
                            </CardDescription>
                        </CardHeader>
                                    ...
                                    <div className="space-y-4">
                                        ...
                                        {taxResult.medicare_surcharge > 0 && (
                                            <div className="flex justify-between items-center">
                                                ...
                                                <span className="font-medium">{formatCurrency(taxResult.medicare_surcharge)}</span>
                                            </div>
                                        )}
                                        {taxResult.lito > 0 && (
                                            ...
                                            <div className="space-y-2">
                                                {taxResult.brackets_breakdown.map((bracket, idx) => (
                                                    <div key={idx} className="flex justify-between text-sm">
                                                        ...
                                                        <span>
                                                            {formatCurrency(bracket.income_in_bracket)} @ {formatCurrency(bracket.tax_for_bracket)}
                                                        </span>
                                                    </div>
                                                ))}
                                            </div>
                ...
                <TabsContent value="deductions" className="space-y-4">
                    ...
                    <Card>
                        <CardHeader>
                            ...
                            <CardDescription>
                                Track your work-related expenses and deductions
                            </CardDescription>
                        </CardHeader>
                        <CardContent>
                            {deductions.length === 0 ? (
                ...
                <TabsContent value="cgt" className="space-y-4">
                    <div className="grid gap-4 md:grid-cols-3">
                        <Card>
                            ...
                            <CardContent>
                                <div className={`text-2xl font-bold ${totalCGT >= 0 ? 'text-destructive' : 'text-green-600'}`}>
                    ...
                    <Card>
                        <CardHeader>
                            ...
                            <CardDescription>
                                Capital gains and losses for {selectedYear}
                            </CardDescription>
                        </CardHeader>
                        <CardContent>
                            {cgtEvents.length === 0 ? (
                                        ...
                                        <div key={e.id} className="flex items-center justify-between p-3 border rounded-lg">
                                            <div>
                                                ...
                                                <div className="flex gap-2 mt-1">
                                                    {e.discountEligible && (
                                                        <Badge variant="secondary">50% Discount</Badge>
                                                    )}
                                                </div>
                                            </div>
                                            <div className="text-right">
                                                <p className={`font-medium ${e.capitalGainNetCents > 0 ? 'text-destructive' : 'text-green-600'}`}>
                            ...
                            {depreciableAssets.length === 0 ? (
                                ...
                                <div className="space-y-4">
                                    {depreciableAssets.map((a) => {
                                        const depreciatedPercent = ((a.purchaseCostCents - a.currentValueCents) / a.purchaseCostCents) * 100;
                                        return (
                        ...
                        <>
                            <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
                                ...
                                <Card>
                                    ...
                                    <CardContent>
                                        <div className={`text-2xl font-bold ${taxSummary.taxRefundCents > 0 ? 'text-green-600' : 'text-destructive'}`}>
                                ...
                                <CardContent>
                                    <div className="space-y-4">
                                        ...
                                        <div className="flex justify-between text-green-600">
                                            ...
                                            <span className="font-medium">-{formatCurrency(taxSummary.totalDeductionsCents)}</span>
                                        </div>
                                        {taxSummary.netCapitalGainCents > 0 && (
                                            <div className="flex justify-between">
                                                <span>Net Capital Gain</span>
                                                <span className="font-medium">{formatCurrency(taxSummary.netCapitalGainCents)}</span>
                                            </div>
                                        )}
                                        ...
                                        <div className="border-t pt-4 flex justify-between text-lg font-bold">
                                            ...
                                            <span className={taxSummary.taxRefundCents > 0 ? 'text-green-600' : 'text-destructive'}>
                                                {formatCurrency(taxSummary.taxRefundCents || taxSummary.taxPayableCents)}
                                            </span>
                                        </div>
                                    </div>
