...
export function TrustReturn({ year }: { year: string }) {
    ...
    if (loading) {
        return (
            <div className="flex items-center justify-center py-12">
                ...
                <span className="ml-2 text-zinc-400">Calculating trust return...</span>
            </div>
        );
    }
    ...
    const hasSection100AWarning = data.warnings.some(w => w.toLowerCase().includes('section 100a'));
                    ...
                    <CardContent className="pt-6 flex items-start gap-3">
                        ...
                        <div>
                            ...
                            <p className="text-sm text-zinc-400 mt-1">
                                Trust distributions that involve reimbursement agreements may be assessed
                                at the top marginal rate under Section 100A ITAA 1936. Review distribution
                                arrangements with your tax adviser.
                            </p>
                        </div>
                    </CardContent>
                </Card>
                ...
                <CardContent>
                    <div className="space-y-3">
                        <div className="flex justify-between">
                            <span className="text-zinc-400">Distributable Amount</span>
                            <span className="font-medium">{formatCurrency(data.taxableIncomeCents)}</span>
                        </div>
                        <p className="text-xs text-zinc-500 pt-2">
                            Undistributed income is taxed at the top marginal rate (45% + 2% Medicare).
                            Ensure a valid resolution is made before year-end.
                        </p>
                    </div>
                </CardContent>
            </Card>
