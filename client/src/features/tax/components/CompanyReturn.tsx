...
export function CompanyReturn({ year }: { year: string }) {
    ...
    useEffect(() => {
        setLoading(true);
        setError(null);
        taxApi.fetchCompanyReturn(year)
            .then(setData)
                ...
                <CardContent>
                    <div className="space-y-3">
                        ...
                        <div className="flex justify-between">
                            ...
                            <span className="font-medium text-emerald-400">{formatCurrency(data.netTaxPayableCents)}</span>
                        </div>
                        <p className="text-xs text-zinc-500 pt-2">
                            Franking credits can be distributed to shareholders with dividends, reducing their personal tax liability.
                        </p>
                    </div>
                </CardContent>
            </Card>
