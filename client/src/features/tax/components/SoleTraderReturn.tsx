...
import { Loader2, Building2, TrendingUp, TrendingDown, DollarSign, AlertTriangle } from 'lucide-react';
...
export function SoleTraderReturn({ year }: { year: string }) {
    ...
    useEffect(() => {
        setLoading(true);
        setError(null);
        taxApi.fetchSoleTraderReturn(year)
            .then(setData)
    ...
    if (loading) {
        return (
            <div className="flex items-center justify-center py-12">
                ...
                <span className="ml-2 text-zinc-400">Calculating sole trader return...</span>
            </div>
        );
    }
