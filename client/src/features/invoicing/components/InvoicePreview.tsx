            ...
            <div className="bg-white text-gray-900 rounded-2xl p-8 shadow-lg max-w-3xl mx-auto print:shadow-none print:rounded-none print:p-0 print:max-w-none">
                ...
                <div className="grid grid-cols-2 gap-8 mb-8">
                    <div>
                        ...
                        {invoice.customerABN && <p className="text-sm text-gray-500">ABN: {invoice.customerABN}</p>}
                    ...
                    <div className="text-right">
                        <div className="space-y-1">
                            ...
                            <div>
                                ...
                                <span className="text-sm font-medium text-gray-700">{formatDate(invoice.dueDate)}</span>
                            </div>
                            {invoice.status && (
                                <div className="mt-2">
                ...
                <table className="w-full mb-8">
                    <thead>
                        <tr className="border-b-2 border-gray-200">
                            <th className="text-left py-2 text-xs font-bold uppercase tracking-wider text-gray-500">Description</th>
                            <th className="text-right py-2 text-xs font-bold uppercase tracking-wider text-gray-500">Qty</th>
                            <th className="text-right py-2 text-xs font-bold uppercase tracking-wider text-gray-500">Unit Price</th>
                            <th className="text-right py-2 text-xs font-bold uppercase tracking-wider text-gray-500">GST</th>
                            <th className="text-right py-2 text-xs font-bold uppercase tracking-wider text-gray-500">Amount</th>
                        </tr>
                    </thead>
                    <tbody>
                        {invoice.lineItems.map((li, i) => (
                            <tr key={i} className="border-b border-gray-100">
                                ...
                                <td className="py-3 text-sm font-medium text-gray-900 text-right">{formatAUD(li.amount + li.gstAmount)}</td>
                            </tr>
                        ))}
                    </tbody>
                </table>
