...
export function InvoiceEditor({ invoiceId, onSave, onCancel }: InvoiceEditorProps) {
    ...
    const [lineItems, setLineItems] = useState<LineItem[]>([
        { id: nextLineId(), description: '', quantity: 1, unitPrice: 0, gstRate: 0.10 },
    ]);

    // Load next invoice number
    useEffect(() => {
        if (!invoiceId) {
            invoicingApi.getNextInvoiceNumber()
    ...
    // Line item helpers
    const addLine = () => {
        setLineItems(prev => [...prev, { id: nextLineId(), description: '', quantity: 1, unitPrice: 0, gstRate: 0.10 }]);
    };
    ...
    const gstTotal = lineItems.reduce((sum, li) => sum + Math.round(li.quantity * li.unitPrice * li.gstRate), 0);
    ...
    return (
        <div className="space-y-6 max-w-4xl">
                    ...
                    <div className="flex items-center justify-between neu-inset rounded-xl p-3">
                        <div>
                            ...
                            <p className="text-xs text-zinc-500">
                                {selectedCustomer.contactName ?? ''} {selectedCustomer.abn ? `| ABN: ${selectedCustomer.abn}` : ''}
                            </p>
                        </div>
                        <button
                            ...
                            <div className="absolute z-20 top-full mt-1 w-full max-h-48 overflow-y-auto rounded-xl bg-[#15161f] border border-zinc-700 shadow-xl">
                                {customers.map(c => (
                                    ...
                                    >
                                        ...
                                        <p className="text-xs text-zinc-500">{c.contactName ?? ''} {c.email ? `| ${c.email}` : ''}</p>
                                    </button>
                                ))}
                            </div>
                        )}
            ...
            {/* Invoice Details */}
            <div className="neu-raised rounded-2xl p-5 space-y-4">
                <h4 className="text-xs font-bold uppercase tracking-wider text-zinc-500">Invoice Details</h4>
                        ...
                        <div key={li.id} className="grid grid-cols-12 gap-2 items-center">
                            ...
                            <div className="col-span-4 sm:col-span-2">
                                <input
                                    type="number"
                                    value={li.quantity}
                                    onChange={e => updateLine(li.id, 'quantity', Math.max(0, parseFloat(e.target.value) || 0))}
                                    min="0"
                                    step="1"
                            ...
                            <div className="col-span-4 sm:col-span-2">
                                <input
                                    ...
                                    onChange={e => updateLine(li.id, 'unitPrice', Math.round((parseFloat(e.target.value) || 0) * 100))}
                                    min="0"
                                    step="0.01"
                                    placeholder="0.00"
                                    className="w-full px-3 py-2 rounded-lg neu-inset bg-transparent text-sm text-zinc-200 text-right focus:outline-none focus:ring-1 focus:ring-[#FFCC00]/30"
                            ...
                            <div className="col-span-3 sm:col-span-2">
                                ...
                                >
                                    <option value={0.10}>10% GST</option>
