...
const statusConfig: Record<string, { bg: string; text: string; label: string; strikethrough?: boolean }> = {
    draft: { bg: 'bg-zinc-600/20', text: 'text-zinc-400', label: 'Draft' },
    ...
    void: { bg: 'bg-zinc-700/50', text: 'text-zinc-500', label: 'Void', strikethrough: true },
};
            ...
            <div className="flex flex-col lg:flex-row gap-3 items-start lg:items-center justify-between">
                <div className="flex flex-wrap gap-3 items-center">
                    <select
                        ...
                        className="px-3 py-2 rounded-xl neu-inset bg-transparent text-sm text-zinc-200 focus:outline-none focus:ring-1 focus:ring-[#FFCC00]/30"
                    >
            ...
            {loading ? (
                ...
                <div className="neu-raised rounded-2xl p-8 text-center">
                    ...
                    <p className="text-zinc-400 text-sm">
                        {statusFilter ? 'No invoices match your filter.' : 'No invoices yet. Create your first invoice.'}
                    </p>
                </div>
            ) : (
                        ...
                        <table className="w-full text-sm">
                            <thead>
                                <tr className="border-b border-zinc-800">
                                    <th className="text-left px-4 py-3 text-[10px] uppercase tracking-wider text-zinc-500 font-semibold">Invoice #</th>
                                    <th className="text-left px-4 py-3 text-[10px] uppercase tracking-wider text-zinc-500 font-semibold">Customer</th>
                                    <th className="text-left px-4 py-3 text-[10px] uppercase tracking-wider text-zinc-500 font-semibold hidden md:table-cell">Issue Date</th>
                                    <th className="text-left px-4 py-3 text-[10px] uppercase tracking-wider text-zinc-500 font-semibold hidden md:table-cell">Due Date</th>
                                    <th className="text-right px-4 py-3 text-[10px] uppercase tracking-wider text-zinc-500 font-semibold">Total</th>
                                    <th className="text-center px-4 py-3 text-[10px] uppercase tracking-wider text-zinc-500 font-semibold">Status</th>
                                    <th className="text-right px-4 py-3 text-[10px] uppercase tracking-wider text-zinc-500 font-semibold">Actions</th>
                                </tr>
                            </thead>
                            <tbody>
                                {invoices.map((inv) => {
                                    ...
                                    return (
                                        ...
                                        >
                                            <td className="px-4 py-3">
                                                <span className={cn(
                                                    "font-semibold text-[#FFCC00]",
                                                    sc.strikethrough && "line-through"
                                                )}>
                                                    {inv.invoiceNumber}
                                                </span>
                                            </td>
                                            <td className={cn("px-4 py-3 text-zinc-200", sc.strikethrough && "line-through")}>
                                                {inv.customerName ?? '—'}
                                            </td>
                                            <td className={cn("px-4 py-3 text-zinc-400 hidden md:table-cell", sc.strikethrough && "line-through")}>
                                            ...
                                            <td className="px-4 py-3 text-right">
                                                <span className={cn(
                                                    "font-semibold",
                                                    sc.strikethrough ? "line-through text-zinc-500" : "text-zinc-100"
                                                )}>
                                                    {formatAUD(inv.total)}
                                                </span>
                                            </td>
