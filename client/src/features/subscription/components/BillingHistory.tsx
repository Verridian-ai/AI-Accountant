...
const STATUS_STYLES: Record<string, { bg: string; text: string }> = {
  ...
  trialing: { bg: 'bg-[#FFCC00]/10', text: 'text-[#FFCC00]' },
...
export function BillingHistory() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-2xl font-bold tracking-tight text-gradient-gold">Billing History</h2>
        <p className="text-sm text-zinc-500">View your subscription payments and invoices</p>
      </div>
                ...
                {records.map((rec) => {
                  ...
                  return (
                    <tr key={rec.id} className="border-b border-white/5 last:border-0 hover:bg-white/[0.02] transition-colors">
                      ...
                      <td className="px-4 py-3 text-right font-bold text-zinc-200 tabular-nums">
