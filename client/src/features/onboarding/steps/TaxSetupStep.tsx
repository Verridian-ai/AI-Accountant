...
const ACCOUNTING_METHODS = [
    ...
    },
    {
        ...
        icon: FileText,
        details: 'Required for businesses with turnover > $10M. GST is reported when invoices are issued.',
    },
] as const;
...
export function TaxSetupStep({ data, updateData }: OnboardingStepProps) {
    ...
    return (
        <div className="p-6 sm:p-10 space-y-8">
                    ...
                    <div className="flex items-start justify-between gap-4">
                        <div className="space-y-1">
                            ...
                            <p className="text-xs text-zinc-500 max-w-md">
                                ...
                                Registration is required if your annual turnover exceeds $75,000.
                            </p>
                        </div>
                        <Switch
                    ...
                    {!data.isRegisteredForGst && (
                        <div className="flex items-start gap-3 p-3 rounded-xl bg-amber-500/10 border border-amber-500/20 animate-in fade-in duration-200">
                            ...
                            <p className="text-xs text-amber-200/80">
                                If your turnover exceeds $75,000, you must register for GST.
                                You can still track GST for reporting purposes without being registered.
                            </p>
                        </div>
                    )}
                </div>
                        ...
                        {BAS_FREQUENCIES.map((freq) => {
                            ...
                            return (
                                ...
                                >
                                    ...
                                    <p className={cn(
                                        "text-xs",
                                        isSelected ? "text-[#0a0a0f]/70" : "text-zinc-500"
                                    )}>
                                        {freq.description}
                                    </p>
                                    <p className={cn(
            ...
            {/* Accounting Method */}
            {showGstOptions && data.isRegisteredForGst && (
                <div className="space-y-4 animate-in fade-in slide-in-from-top-4 duration-300" style={{ animationDelay: '100ms' }}>
                    <div className="flex items-center gap-2">
                        <label className="text-[10px] font-black uppercase text-zinc-500 tracking-widest">
                            Accounting Method
                        </label>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        {ACCOUNTING_METHODS.map((method) => {
                            ...
                            return (
                                <button
                                    key={method.id}
                                    onClick={() => updateData({ accountingMethod: method.id as typeof data.accountingMethod })}
                                    className={cn(
            ...
            {!showGstOptions && (
                <div className="neu-inset rounded-2xl p-5 border border-[#FFCC00]/10 bg-[#FFCC00]/[0.02]">
                    <p className="text-xs text-zinc-400 leading-relaxed">
                        <span className="text-[#FFCC00] font-bold">For individuals:</span>
                        {' '}GST and BAS settings don't apply to personal finances.
                        We'll focus on tracking your income and deductible expenses for tax time.
                    </p>
                </div>
            )}
        </div>
