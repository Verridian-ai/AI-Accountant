  ...
  return (
    <div className="p-6 sm:p-10 flex flex-col items-center text-center">
      ...
      <div className="relative mb-8">
        <div className="absolute inset-0 bg-cba-gold blur-3xl opacity-20 animate-pulse-glow rounded-full" />
        <div className="relative w-24 h-24 sm:w-32 sm:h-32 neu-raised rounded-[2rem] flex items-center justify-center border border-cba-gold/20">
          <Activity className="w-12 h-12 sm:w-16 sm:h-16 text-cba-gold" />
        </div>
      ...
      <p className="text-lg sm:text-xl text-secondary font-medium mb-2">Statement Parser</p>
      <p className="text-sm text-muted max-w-md mb-10">
...
function FeatureCard({ icon: Icon, title, description, delay }: FeatureCardProps) {
  return (
    <div
      className={cn(
        'neu-raised-sm p-5 rounded-2xl text-left border border-border/50',
        'animate-in fade-in slide-in-from-bottom-4 duration-500',
      )}
      style={{ animationDelay: `${delay}ms` }}
    >
      <div className="neu-inset p-2 rounded-xl w-fit mb-3 text-cba-gold">
