            ...
            {recommendedGoals.length > 0 && (
                <div className="neu-inset rounded-2xl p-5 border border-[#FFCC00]/10 bg-[#FFCC00]/[0.02]">
                    ...
                    <p className="text-xs text-zinc-400">
                        Based on your profile as {data.entityType === 'individual' ? 'an individual' : `a ${data.entityType.replace('_', ' ')}`}
                        {data.isRegisteredForGst && ' registered for GST'}, we recommend focusing on:{' '}
                        <span className="text-zinc-200">
                            {recommendedGoals.map(id => {
                ...
                {FINANCIAL_GOALS.map((goal) => {
                    ...
                    return (
                        ...
                        >
                            ...
                            <p className="text-xs text-zinc-500 pr-8">
                                {goal.description}
                            </p>
                        </button>
                    );
