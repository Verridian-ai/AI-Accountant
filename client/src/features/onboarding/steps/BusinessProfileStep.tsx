...
export function BusinessProfileStep({ data, updateData }: OnboardingStepProps) {
    ...
    return (
        <div className="p-6 sm:p-10 space-y-8">
            ...
            {/* ABN */}
            {showBusinessFields && (
                <div className="space-y-3 animate-in fade-in slide-in-from-top-4 duration-300" style={{ animationDelay: '50ms' }}>
                    <label className="text-[10px] font-black uppercase text-zinc-500 tracking-widest ml-1 flex items-center gap-2">
                        ABN
                    ...
                    />
                    {abnError && (
                        <p className="text-xs text-red-400 ml-1">{abnError}</p>
                    )}
                </div>
            )}
            ...
            <div className="neu-inset rounded-2xl p-5 border border-[#FFCC00]/10 bg-[#FFCC00]/[0.02]">
                <p className="text-xs text-zinc-400 leading-relaxed">
                    <span className="text-[#FFCC00] font-bold">Why do we ask?</span>
                    {' '}Your entity type affects tax calculations, GST obligations, and the categories we suggest.
                    {showBusinessFields
