  ...
  const completionItems = useMemo(() => {
    const items = [
      ...
      { label: 'Statement uploaded', completed: data.hasUploadedStatement },
      {
        ...
        completed: data.entityType === 'individual' || data.financialYearEnd !== undefined,
      },
    ...
    <div className="p-6 sm:p-10 space-y-8">
      ...
      <div className="neu-raised rounded-2xl p-6 border border-white/5">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
          {completionItems.map((item, index) => (
            ...
            >
              <CheckCircle2
                className={cn('w-4 h-4', item.completed ? 'text-emerald-400' : 'text-zinc-700')}
              />
              {item.label}
            </div>
          ))}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        <SummaryCard
          icon={Target}
          title="Goals"
          items={selectedGoalLabels.length > 0 ? selectedGoalLabels : ['No goals selected']}
