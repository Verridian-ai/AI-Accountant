...
import { Bell, Mail, Radio, Globe, Plus, Trash2, ToggleLeft, ToggleRight, Edit3, X } from 'lucide-react';
      ...
      {showForm && (
        <div className="neu-raised p-4 rounded-lg space-y-3 border border-[#FFCC00]/20">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            ...
            <div>
              <label className="text-xs text-zinc-500 font-medium block mb-1">Severity Filter</label>
              <select
                value={formSeverity}
      ...
      {subscriptions.length === 0 && !showForm ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          {subscriptions.map(sub => {
            ...
            return (
              <div key={sub.id} className={cn('neu-raised p-4 rounded-lg border-l-4 transition-all', sub.isActive ? 'border-[#FFCC00]' : 'border-zinc-700')}>
                <div className="flex items-start justify-between">
                  <div className="flex items-center gap-3">
                    <div className={cn('p-2 rounded-lg', sub.isActive ? 'bg-[#FFCC00]/10' : 'bg-zinc-800')}>
                      <ChannelIcon className={cn('w-5 h-5', sub.isActive ? 'text-[#FFCC00]' : 'text-zinc-600')} />
                    </div>
                    <div>
                ...
                {sub.filterCriteria && Object.keys(sub.filterCriteria).length > 0 && (
                  <div className="flex flex-wrap gap-1 mt-2">
                    {Object.entries(sub.filterCriteria).map(([k, v]) => (
                      <span key={k} className="px-2 py-0.5 rounded-full text-[9px] bg-zinc-800 text-zinc-400 border border-zinc-700">
                        {k}: {String(v)}
                      </span>
                    ))}
                  </div>
