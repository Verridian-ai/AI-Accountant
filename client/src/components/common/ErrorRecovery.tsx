...
interface ErrorRecoveryProps {
    errorType: 'PDF_READ_ERROR' | 'AI_PARSE_ERROR' | 'EMPTY_STATEMENT' | 'CRITICAL_ERROR' | null | undefined;
    onRetry?: () => void;
    onManualFix?: () => void;
...
export function ErrorRecovery({ errorType, onRetry, onManualFix, onContactSupport, isRetrying }: ErrorRecoveryProps) {
    const getRecoveryOptions = () => {
        switch (errorType) {
            case 'PDF_READ_ERROR':
                return {
                    title: "File Integrity Issue",
                    description: "The PDF could not be read clearly. It might be password protected, corrupted, or an image scan.",
                    tips: [
