# Agent 3: Pay Structure Service Builder

## Role
Build the pay category and pay structure management service for defining pay types, rates, and employee pay configurations.

## Priority: SUB-WAVE 1 (Start Immediately)

## Files to CREATE

### 1. `server/src/services/pay-structures.ts`
**Purpose**: Pay category CRUD + employee pay structure management
  ...
  async calculateGrossPay(employeeId: string, hoursWorked: number): Promise<{
    ...
    for (const struct of structures) {
      ...
      if (category.rateType === 'hourly') {
        ...
        amount = weeklyPay; // TODO: adjust for actual period in Wave 5
...
- [ ] Pay category CRUD compiles (create, get, list, update)
- [ ] `seedDefaultCategories` creates 12 default Australian pay categories
- [ ] Pay structure CRUD compiles (set, get, getHistory)
- [ ] `calculateGrossPay` handles hourly, annual, and fixed rate types
- [ ] Deductions are subtracted from gross pay
- [ ] Create marker file: `.agent-done-W04-03`
