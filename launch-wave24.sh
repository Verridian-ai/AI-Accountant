#!/bin/bash
# ============================================================
# GoldLedger — Wave 24: Mobile Responsive & PWA
# ============================================================
# Spawns Claude Code with Agent Teams in a tmux session.
# Run from WSL: bash launch-wave24.sh
# ============================================================
...
TMUX_SESSION="goldledger-wave24"
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"

echo -e "${CYAN}============================================================${NC}"
echo -e "${CYAN}  GoldLedger — Wave 24 Agent Team Launcher${NC}"
echo -e "${CYAN}  Mobile Responsive & PWA${NC}"
echo -e "${CYAN}============================================================${NC}"
echo ""
...
# Step 3: Verify Wave 22 + Wave 23 completion
echo -e "${YELLOW}[3/5] Checking Wave 22 + Wave 23 prerequisites...${NC}"
if [ -f "$PROJECT_DIR/.agent-done-wave22" ]; then
    echo -e "${GREEN}  ✓ Wave 22 completed${NC}"
else
    echo -e "${YELLOW}  ⚠ Wave 22 marker not found — proceeding anyway${NC}"
fi
if [ -f "$PROJECT_DIR/.agent-done-wave23" ]; then
    echo -e "${GREEN}  ✓ Wave 23 completed${NC}"
else
    echo -e "${YELLOW}  ⚠ Wave 23 marker not found — proceeding anyway${NC}"
fi
...
tmux set-option -t "$TMUX_SESSION" -g status-left "#[fg=#f7768e,bold] Wave 24: Mobile Responsive & PWA "
